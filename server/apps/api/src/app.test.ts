...
const ready = async (): Promise<boolean> => true;

describe("API foundation", () => {
  it("serves liveness", async () => {
    const app = createApiApp({
      config,
      isInfrastructureReady: async () => false,
    });
  ...
  it("reports ready when infrastructure is ready", async () => {
    const app = createApiApp({ config, isInfrastructureReady: ready });
  ...
  it("uses the stable error envelope when infrastructure is not ready", async () => {
    ...
    expect(ApiErrorEnvelopeV1Schema.parse(response.json()).error.code).toBe(
      "SERVICE_UNAVAILABLE",
    );
    await app.close();
  });
  it("generates and preserves allowed correlation IDs", async () => {
    ...
    const preserved = await app.inject({
      url: "/health/live",
      headers: { "x-request-id": "accepted-request-id-1" },
    });
  ...
  it("uses the stable controlled error envelope", async () => {
    ...
    app.get("/test-controlled-error", () => {
      throw new ControlledError("INVALID_REQUEST", "Fixture rejected", 400);
    });
