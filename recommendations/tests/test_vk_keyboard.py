...
from recommendations.vk.keyboard import gender_keyboard, main_menu_keyboard, restart_keyboard
...
class KeyboardTests(unittest.TestCase):
    ...
    def test_exact_builders(self):
        self.assertEqual(gender_keyboard(), {"one_time":False,"inline":True,"buttons":[[{"action":{"type":"text","label":"Мужчине","payload":"{\"kip\":\"gender\",\"value\":\"male\",\"v\":1}"},"color":"primary"},{"action":{"type":"text","label":"Женщине","payload":"{\"kip\":\"gender\",\"value\":\"female\",\"v\":1}"},"color":"secondary"}]]})
        self.assertEqual(main_menu_keyboard(), {"one_time":False,"inline":False,"buttons":[[{"action":{"type":"text","label":"Подобрать оберег","payload":"{\"kip\":\"menu\",\"value\":\"recommend\",\"v\":1}"},"color":"primary"}],[{"action":{"type":"text","label":"Задать вопрос","payload":"{\"kip\":\"menu\",\"value\":\"human\",\"v\":1}"},"color":"secondary"}]]})
        self.assertEqual(restart_keyboard()["buttons"][0][0]["action"]["payload"], '{"kip":"restart","v":1}')
    def test_strict_payload_matrix(self):
        self.assertEqual(parse_keyboard_payload('{"kip":"gender","value":"male","v":1}',"Мужчине"),("gender","male")); self.assertEqual(parse_keyboard_payload('{"kip":"gender","value":"female","v":1}',"Женщине"),("gender","female")); self.assertEqual(parse_keyboard_payload('{"kip":"restart","v":1}',"Подобрать снова"),("restart",None)); self.assertEqual(parse_keyboard_payload('{"kip":"menu","value":"recommend","v":1}',"Подобрать оберег"),("menu","recommend")); self.assertEqual(parse_keyboard_payload('{"kip":"menu","value":"human","v":1}',"Задать вопрос"),("menu","human"))
    ...
    def test_state_aware_payloads_and_real_restart_fixture(self):
        self.process("Подобрать оберег","start",'{"kip":"menu","value":"recommend","v":1}'); self.process("13.10.1990","date"); self.assertEqual(self.s.session(1,11)['state'],'WAITING_GENDER')
        ...
        result_keyboard=json.loads(self.s.connection.execute("select keyboard_json from vk_outbox order by outbox_id desc limit 1").fetchone()[0])
        self.assertEqual((result_keyboard["inline"], len(result_keyboard["buttons"]), [len(row) for row in result_keyboard["buttons"]]), (True, 3, [1, 1, 1]))
