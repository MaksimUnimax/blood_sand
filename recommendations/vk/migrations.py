...
def initialize(connection: sqlite3.Connection) -> None:
    ...
    try:
        ...
        if 4 not in versions:
            columns = {row[1] for row in connection.execute("PRAGMA table_info(vk_outbox)")}
            if "keyboard_audit_json" not in columns:
                connection.execute("ALTER TABLE vk_outbox ADD COLUMN keyboard_audit_json TEXT NULL")
            connection.executescript("""
            CREATE TABLE IF NOT EXISTS vk_transition_audit(
              ...
              FOREIGN KEY(source_event_id) REFERENCES vk_inbound_events(id)
            );
            """)
            connection.execute("INSERT INTO vk_schema_migrations(version,applied_at) VALUES(4,datetime('now'))")
        if 5 not in versions:
            columns = {row[1] for row in connection.execute("PRAGMA table_info(vk_bot_sessions)")}
            for column, kind in (("date_picker_step", "TEXT"), ("date_picker_range_start", "INTEGER"), ("date_picker_range_end", "INTEGER")):
        ...
        if 7 not in versions:
            columns = {row[1] for row in connection.execute("PRAGMA table_info(vk_outbox)")}
            if "attachment_json" not in columns:
                connection.execute("ALTER TABLE vk_outbox ADD COLUMN attachment_json TEXT NULL")
            if "attachment_audit_json" not in columns:
                connection.execute("ALTER TABLE vk_outbox ADD COLUMN attachment_audit_json TEXT NULL")
            connection.execute("INSERT INTO vk_schema_migrations(version,applied_at) VALUES(7,datetime('now'))")
        if 8 not in versions:
            connection.executescript("""
            CREATE TABLE IF NOT EXISTS vk_miniapp_standalone_sessions(
              session_id TEXT PRIMARY KEY,
              session_token_hash TEXT UNIQUE NOT NULL,
              launch_fingerprint TEXT NULL,
              vk_app_id INTEGER NOT NULL,
              verified_vk_user_id INTEGER NOT NULL,
              created_at TEXT NOT NULL,
            ...
            CREATE INDEX IF NOT EXISTS vk_miniapp_standalone_sessions_fingerprint
              ON vk_miniapp_standalone_sessions(launch_fingerprint);
            """)
            connection.execute("INSERT INTO vk_schema_migrations(version,applied_at) VALUES(8,datetime('now'))")
        connection.commit()
    except Exception:
