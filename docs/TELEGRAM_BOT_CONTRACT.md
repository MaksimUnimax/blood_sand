...
`app.acceptance` is an isolated, opt-in harness, not a production mode. A clean run uses `/var/lib/marketplace-question-operator/t4-runs/<run-id>/`; its run id is an opaque ASCII `[A-Za-z0-9_-]{1,64}` identifier, validated by the application before canonical paths are derived. Raw systemd instance text is never a filesystem path. The database and evidence paths must resolve directly beneath that root; production and historical `t4-acceptance` paths remain invalid. Startup sends no cards and creates no questions. `prepare` creates exactly one explicit scenario (`A_MANUAL`, `B_CODEX_SUCCESS`, `C_CODEX_ERROR_REPEAT`, `D_CODEX_ERROR_SWITCH`, or `E_IGNORE`); SQLite serializes preparation and enforces one ACTIVE row until it is closed. `status --run-id` uses the same resolver, is read-only, and never polls or sends Telegram.

Callback data is a versioned canonical, fail-closed schema per action. It is measured in UTF-8 bytes and is compactly encoded so full positive SQLite INTEGER ids remain renderable within Telegram's 64-byte callback limit.
