...
RAW='b67665029fc8ae962aa390178fadc210f403e9ceac77107068f049a64f148321'
GZ='20ea8158edadd35445099fe9bf4431de76e4f14f3a26b6cf51e150c11ecc078f'
COUNT=21
CHANGED={'shared/ozon_entitlements.js': 'e662ac981bbc1e9c305a00743a91ebf422dac52d4fc07790399116e84dd58fb7', 'shared/ozon_operation_registry.js': '6e4f4bb5a7d0ddd0350cd3edecfbdea806c6b51ec05e708ed6cff483d69c9470', 'shared/ozon_contract.js': '18c2d80bbada1b12ce57cf7aec2686020b4f8cde45952a5dfef94eb0a6186887'}
...
def main():
 ...
 b20=v/'materialize_patch_b20_catalog_certificate_data_reads_candidate.py'; gz=v/'PATCH_B21_RETURN_GIVEOUT_READS_2026-08-27.patch.gz'
 if not b20.is_file(): raise RuntimeError(f'missing B20 materializer: {b20}')
 if not gz.is_file(): raise RuntimeError(f'missing B21 patch: {gz}')
 comp=gz.read_bytes()
 if sha(comp)!=GZ: raise RuntimeError('B21 gzip identity mismatch')
 raw=gzip.decompress(comp)
 if sha(raw)!=RAW: raise RuntimeError('B21 raw patch identity mismatch')
 if work.exists(): shutil.rmtree(work)
 work.mkdir(parents=True); b20work=work/'b20-work'; b20out=work/'b20-base'
 subprocess.run([sys.executable,str(b20),'--repo-root',str(repo),'--work-root',str(b20work),'--out',str(b20out)],check=True)
 if tree(b20out)!=B20_TREE: raise RuntimeError('B20 base identity mismatch')
 if out.exists(): shutil.rmtree(out)
 shutil.copytree(b20out,out)
 ...
 if r.returncode: raise RuntimeError('B21 patch apply failed:\n'+r.stderr.decode(errors='replace'))
 files=[p for p in out.rglob('*') if p.is_file()]
 if len(files)!=COUNT: raise RuntimeError(f'B21 file count {len(files)} != {COUNT}')
 for rel,exp in CHANGED.items():
  got=sha((out/rel).read_bytes())
  if got!=exp: raise RuntimeError(f'B21 changed identity mismatch {rel}: {got} != {exp}')
 for rel,exp in PROTECTED.items():
  got=sha((out/rel).read_bytes())
  if got!=exp: raise RuntimeError(f'B21 protected B20 identity mismatch {rel}: {got} != {exp}')
 got=tree(out)
 if got!=B21_TREE: raise RuntimeError(f'B21 tree mismatch {got} != {B21_TREE}')
 for m in ['PATCH_B21_B20_BASE_IDENTITY_PASS','PATCH_B21_PATCH_TRANSPORT_IDENTITY_PASS','PATCH_B21_PATCH_APPLY_PASS','PATCH_B21_PRODUCTION_FILE_COUNT_21_PASS','PATCH_B21_CHANGED_FILE_IDENTITIES_PASS','PATCH_B21_PROTECTED_B20_IDENTITIES_PASS','PATCH_B21_TREE_MANIFEST_SHA256_PASS']: print(m)
 print(out)
