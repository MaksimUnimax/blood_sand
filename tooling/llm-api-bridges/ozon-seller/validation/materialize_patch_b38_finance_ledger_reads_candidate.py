...
B37='a1fc2f298cb1916039b3cb6fed6af23e62f8cda1eed6df26ccd8bf3e5785539e';B38='8011faa572366f8d9de3bf7cfcbc198a00e93140471a7d9624a4461b425b48bf';RAW='d1ff737b3c4226ce35b3331654f814696b5e4a276d6ea855f68b80e3e81e3167';GZ='62b6ae9beb5f757326af31562b32b3b6f8e350efbe308a607d5cb33671c5137d'
CH={'shared/ozon_operation_registry.js':'7e58e4082cfb81efb624106aa824b6e1822517c0cb4ea31e4d6ce811c592f75a','shared/ozon_contract.js':'bf4d286e48dda4c3cbc18991d5df88b390960e1bbf7d18bda57944bf16e847e1','shared/ozon_entitlements.js':'50296e8c7443d52174c77d3ef331eb8fa01f2319afde0373cd3d9202419c69dc'}
...
def tree(r):
 ...
 return sha(s.encode())
def main():
 a=argparse.ArgumentParser();a.add_argument('--repo-root',required=True);a.add_argument('--work-root',required=True);a.add_argument('--out',required=True);x=a.parse_args();repo=Path(x.repo_root).resolve();work=Path(x.work_root).resolve();out=Path(x.out).resolve();v=repo/'tooling/llm-api-bridges/ozon-seller/validation';prev=v/'materialize_patch_b37_fbo_removal_report_reads_candidate.py';pg=v/'PATCH_B38_FINANCE_LEDGER_READS_2026-08-28.patch.gz';c=pg.read_bytes();assert sha(c)==GZ;raw=gzip.decompress(c);assert sha(raw)==RAW
 if work.exists():shutil.rmtree(work)
 work.mkdir(parents=True);base=work/'b37-base'
 if sys.platform.startswith('win'):
  prev_work=Path('C:/w38');prev_out=Path('C:/b37')
  for p in (prev_work,prev_out):
   if p.exists():shutil.rmtree(p)
 else:prev_work=work/'b37-work';prev_out=base
 subprocess.run([sys.executable,str(prev),'--repo-root',str(repo),'--work-root',str(prev_work),'--out',str(prev_out)],check=True);assert tree(prev_out)==B37
 if prev_out!=base:
  if base.exists():shutil.rmtree(base)
  shutil.copytree(prev_out,base);assert tree(base)==B37
 ...
 assert tree(out)==B38
 for m in ['PATCH_B38_B37_BASE_IDENTITY_PASS','PATCH_B38_PATCH_TRANSPORT_IDENTITY_PASS','PATCH_B38_PATCH_APPLY_PASS','PATCH_B38_PRODUCTION_FILE_COUNT_21_PASS','PATCH_B38_CHANGED_FILE_IDENTITIES_PASS','PATCH_B38_PROTECTED_B37_IDENTITIES_PASS','PATCH_B38_TREE_MANIFEST_SHA256_PASS']:print(m)
