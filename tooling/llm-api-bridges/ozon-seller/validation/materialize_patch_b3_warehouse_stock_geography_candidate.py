...
B3_PATCH_SHA256 = "d5314063b91be87045c42935e259a7731fdc1cacf290cfda1a2035dc238d1b4f"
...
def main() -> None:
    if len(sys.argv) != 3:
        raise SystemExit("usage: materialize_patch_b3_warehouse_stock_geography_candidate.py <repo-root> <output-dir>")
    repo = Path(sys.argv[1]).resolve()
    out = Path(sys.argv[2]).resolve()
    validation = repo / "tooling/llm-api-bridges/ozon-seller/validation"
    b2 = validation / "materialize_patch_b2_prices_listing_state_candidate.py"
    patch = validation / "PATCH_B3_WAREHOUSE_STOCK_GEOGRAPHY_2026-08-26.patch"
    if not b2.is_file():
        raise RuntimeError(f"missing exact B2 materializer: {b2}")
    if not patch.is_file():
        raise RuntimeError(f"missing B3 patch: {patch}")
    patch_bytes = patch.read_bytes()
    if sha256(patch_bytes) != B3_PATCH_SHA256:
        raise RuntimeError("B3 patch identity mismatch")

    if out.exists():
        shutil.rmtree(out)
    subprocess.run([sys.executable, str(b2), str(repo), str(out)], check=True)
    if tree_digest(out) != B2_TREE_MANIFEST_SHA256:
        raise RuntimeError("B2 base tree identity mismatch before B3")
    ...
    if applied.returncode != 0:
        raise RuntimeError(f"B3 patch apply failed:\n{applied.stderr.decode('utf-8', errors='replace')}")

    files = [p for p in out.rglob("*") if p.is_file()]
    if len(files) != EXPECTED_FILE_COUNT:
        raise RuntimeError(f"B3 production file count {len(files)} != {EXPECTED_FILE_COUNT}")
    for rel, expected in EXPECTED_CHANGED_SHA256.items():
        actual = sha256((out / rel).read_bytes())
        if actual != expected:
            raise RuntimeError(f"B3 changed identity mismatch {rel}: {actual} != {expected}")
    for rel, expected in PROTECTED_B2_SHA256.items():
        actual = sha256((out / rel).read_bytes())
        if actual != expected:
            raise RuntimeError(f"B3 protected B2 identity mismatch {rel}: {actual} != {expected}")
    actual_tree = tree_digest(out)
    if actual_tree != B3_TREE_MANIFEST_SHA256:
        raise RuntimeError(f"B3 tree identity {actual_tree} != {B3_TREE_MANIFEST_SHA256}")

    print("PATCH_B3_B2_BASE_IDENTITY_PASS")
    print("PATCH_B3_PATCH_IDENTITY_PASS")
    print("PATCH_B3_PATCH_APPLY_PASS")
    print("PATCH_B3_PRODUCTION_FILE_COUNT_21_PASS")
    print("PATCH_B3_CHANGED_FILE_IDENTITIES_PASS")
    print("PATCH_B3_PROTECTED_B2_IDENTITIES_PASS")
    print("PATCH_B3_TREE_MANIFEST_SHA256_PASS")
    print(str(out))
