...
B5_PATCH_SHA256 = "eb8d3fa8b8347ab58b363460e99b17fe2c0c014ac46158a5d58377fe561afce5"
B5_PATCH_GZIP_SHA256 = "c564134bfc330930d8f23f805424d897a68ef97ca421d81d5f3e2b7394b2b6a5"
...
def reconstruct_patch(patch_gzip: Path) -> bytes:
    ...
    if sha256(compressed) != B5_PATCH_GZIP_SHA256:
        raise RuntimeError("B5 patch gzip identity mismatch")
    try:
        patch = gzip.decompress(compressed)
    except Exception as exc:
        raise RuntimeError(f"B5 patch gzip decode failed: {exc}") from exc
    if sha256(patch) != B5_PATCH_SHA256:
        raise RuntimeError("B5 reconstructed patch identity mismatch")
    return patch
...
def main() -> None:
    ...
    b4 = validation / "materialize_patch_b4_orders_returns_cancellations_candidate.py"
    patch_gzip = validation / "PATCH_B5_FINANCE_REALIZATION_2026-08-26.patch.gz"
    if not b4.is_file():
        raise RuntimeError(f"missing exact B4 materializer: {b4}")
    if not patch_gzip.is_file():
        raise RuntimeError(f"missing B5 patch gzip: {patch_gzip}")
    ...
    b4_out = work / "b4-base"
    subprocess.run([sys.executable, str(b4), str(repo), str(b4_out)], check=True)
    if tree_digest(b4_out) != B4_TREE_MANIFEST_SHA256:
        raise RuntimeError("B4 base tree identity mismatch before B5")

    if out.exists():
        shutil.rmtree(out)
    shutil.copytree(b4_out, out)
    ...
    if applied.returncode != 0:
        raise RuntimeError(f"B5 patch apply failed:\n{applied.stderr.decode('utf-8', errors='replace')}")

    files = [p for p in out.rglob("*") if p.is_file()]
    if len(files) != EXPECTED_FILE_COUNT:
        raise RuntimeError(f"B5 production file count {len(files)} != {EXPECTED_FILE_COUNT}")
    for rel, expected in EXPECTED_CHANGED_SHA256.items():
        actual = sha256((out / rel).read_bytes())
        if actual != expected:
            raise RuntimeError(f"B5 changed identity mismatch {rel}: {actual} != {expected}")
    for rel, expected in PROTECTED_B4_SHA256.items():
        actual = sha256((out / rel).read_bytes())
        if actual != expected:
            raise RuntimeError(f"B5 protected B4 identity mismatch {rel}: {actual} != {expected}")
    actual_tree = tree_digest(out)
    if actual_tree != B5_TREE_MANIFEST_SHA256:
        raise RuntimeError(f"B5 tree identity {actual_tree} != {B5_TREE_MANIFEST_SHA256}")

    print("PATCH_B5_B4_BASE_IDENTITY_PASS")
    print("PATCH_B5_PATCH_TRANSPORT_IDENTITY_PASS")
    print("PATCH_B5_PATCH_APPLY_PASS")
    print("PATCH_B5_PRODUCTION_FILE_COUNT_21_PASS")
    print("PATCH_B5_CHANGED_FILE_IDENTITIES_PASS")
    print("PATCH_B5_PROTECTED_B4_IDENTITIES_PASS")
    print("PATCH_B5_TREE_MANIFEST_SHA256_PASS")
    print(str(out))
