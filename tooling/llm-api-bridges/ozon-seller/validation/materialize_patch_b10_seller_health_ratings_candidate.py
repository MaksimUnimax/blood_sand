...
B9_TREE='d955dbfd1a667e40ea0cb04374b31e0cfe95bbf75b3b355d11c50d10f748a6d5'
B10_TREE='b5af358d19c5e4a720b34f61a6487a20bc07c82c7689a205fde96853c26d46b6'
RAW='44b5527e0cbde268c0e4d6cb378def971369f9815d6a633dae86947a6c68bed3'
GZ='c0a1a486c8a28d6ccfed9338eab6f71f258bf2adf3fd1c0e12a77ac124aec4ea'
COUNT=21
CHANGED={
...
def main():
 ...
 b9=v/'materialize_patch_b9_reviews_questions_candidate.py'; gz=v/'PATCH_B10_SELLER_HEALTH_RATINGS_2026-08-26.patch.gz'
 if not b9.is_file(): raise RuntimeError(f'missing B9 materializer: {b9}')
 if not gz.is_file(): raise RuntimeError(f'missing B10 patch: {gz}')
 comp=gz.read_bytes();
 if sha(comp)!=GZ: raise RuntimeError('B10 gzip identity mismatch')
 raw=gzip.decompress(comp)
 if sha(raw)!=RAW: raise RuntimeError('B10 raw patch identity mismatch')
 if work.exists(): shutil.rmtree(work)
 work.mkdir(parents=True); b9work=work/'b9-work'; b9out=work/'b9-base'
 subprocess.run([sys.executable,str(b9),'--repo-root',str(repo),'--work-root',str(b9work),'--out',str(b9out)],check=True)
 if tree(b9out)!=B9_TREE: raise RuntimeError('B9 base identity mismatch')
 if out.exists(): shutil.rmtree(out)
 shutil.copytree(b9out,out)
 ...
 if r.returncode: raise RuntimeError('B10 patch apply failed:\n'+r.stderr.decode(errors='replace'))
 files=[p for p in out.rglob('*') if p.is_file()]
 if len(files)!=COUNT: raise RuntimeError(f'B10 file count {len(files)} != {COUNT}')
 for rel,exp in CHANGED.items():
  got=sha((out/rel).read_bytes());
  if got!=exp: raise RuntimeError(f'B10 changed identity mismatch {rel}: {got} != {exp}')
 for rel,exp in PROTECTED.items():
  got=sha((out/rel).read_bytes());
  if got!=exp: raise RuntimeError(f'B10 protected B9 identity mismatch {rel}: {got} != {exp}')
 got=tree(out)
 if got!=B10_TREE: raise RuntimeError(f'B10 tree mismatch {got} != {B10_TREE}')
 for m in ['PATCH_B10_B9_BASE_IDENTITY_PASS','PATCH_B10_PATCH_TRANSPORT_IDENTITY_PASS','PATCH_B10_PATCH_APPLY_PASS','PATCH_B10_PRODUCTION_FILE_COUNT_21_PASS','PATCH_B10_CHANGED_FILE_IDENTITIES_PASS','PATCH_B10_PROTECTED_B9_IDENTITIES_PASS','PATCH_B10_TREE_MANIFEST_SHA256_PASS']: print(m)
 print(out)
