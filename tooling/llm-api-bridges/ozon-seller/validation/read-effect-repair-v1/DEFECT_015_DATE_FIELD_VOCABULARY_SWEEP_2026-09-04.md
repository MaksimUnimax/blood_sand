...
Previous passes exhausted:

- `requireRfc3339DateTime()` consumers;
- `requireDateYmd()` consumers;
- `requireAnalyticsDate()`;
- direct permissive `Date.parse()` / `new Date()` paths;
- schema-driven `format: date`, `format: date-time`, `format: month`.

That still does not prove full coverage because a bespoke normalizer may validate a date as a plain string or may reach the strict helper indirectly through `validateFromToObject()`.

The field-level sweep therefore searches the executable authority for date/time vocabulary including `date_from`, `date_to`, camelCase date fields, `since/to`, `created_*`, `published_*`, `processed_at_*`, `cutoff_*`, `delivering_date_*`, `interval_*`, `departure_date`, `issue_date`, `auto_add_date`, return time filters and day/month/year structures.
...
This operation is now explicitly accounted for even though its date fields are reached indirectly through the generic range-object helper.
...
The range helper itself permits a partially supplied optional range unless `requirePair` is explicitly requested. No provider defect is claimed for this operation unless current provider evidence proves that both boundaries must always be supplied together.

## 3. `carriage_delivery_list_v2.departure_date` — NEW confirmed shared-class defect
...
The normalizer validates lexical shape only. It does not round-trip the value as a real calendar date and therefore locally accepts impossible values such as:

- `2026-02-31`;
- `2026-13-40` if the lexical two-digit shape is satisfied.

The second example demonstrates that this validator does not even constrain month/day numeric domains; the first demonstrates the more important invariant: it does not prove representable calendar semantics.
...
This is the same process class as the shared schema `format: date` defect: regex shape is not sufficient to certify a calendar date.
...
Despite belonging to the returns domain, its current Bridge request surface contains only:

- optional `filter.place_id`;
- pagination (`last_id`, `limit`).

There is no request date/time/period field in the authority normalizer.

Classification: **NOT_DATE_RELATED**.

This prevents confusion with separate FBS return-list endpoints that do contain date ranges.

## 5. Previously known vocabulary matches reconfirmed by this pass

The field search also reconfirmed already persisted date-bearing families rather than exposing new defects:
...
## 6. Remaining field-vocabulary queue

Continue field-level source enumeration for bespoke date structures that may not use the named helpers, including:

1. return/FBS company and giveout families;
2. supply/cargo/timeslot/status structures;
3. any raw `date` field whose normalizer uses `requireString` or a lexical regex;
4. day/month/year integer triplets outside the already-audited finance realization methods;
5. generated schema objects outside `EFFECT_REPAIR_PARAM_SCHEMAS`;
6. all operation registry entries that remain after date-bearing operations are fully enumerated, terminally classifying the remainder `NOT_DATE_RELATED`.

A terminal full-registry accounting must be produced before claiming the full 271 Seller-read Bridge surface closed.
