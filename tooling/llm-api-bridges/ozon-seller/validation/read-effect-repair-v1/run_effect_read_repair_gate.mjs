...
const seller = sellerAll.filter(([,meta]) => isCurrentRead(meta));
const performance = entries.filter(([,meta]) => providerOf(meta) === "performance_api" && isCurrentRead(meta));
const sellerNonCurrent = sellerAll.filter(([,meta]) => !isCurrentRead(meta)).map(([alias,meta]) => ({ alias, effect: meta?.effect ?? null, currentness: meta?.currentness ?? null, enabled: meta?.execution_enabled ?? null, path: meta?.path ?? null, privacy_policy: meta?.privacy_policy ?? null }));
...
console.log("EFFECT_REPAIR_PARTITION", JSON.stringify({ total: entries.length, seller_all: sellerAll.length, seller_current_read: seller.length, performance_current_read: performance.length }));
console.log("EFFECT_REPAIR_SELLER_NON_CURRENT", JSON.stringify(sellerNonCurrent));
...
console.log("OZON_EFFECT_READ_REPAIR_SELLER_271_PASS");
