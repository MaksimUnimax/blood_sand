...
RAW='6fc9bdba124d82c4da224d3df7fade9ecab3f1efee5aeb8af51036f63810b393'
GZ='35d2aefdeef07dd6f22caf248336bb3ad764be6e6d84b57a4e33717098f0af24'
CH={'shared/ozon_operation_registry.js':'75f9dba665ef821b465d2d9a99342791b372f9d75d695745ae95d8951e3279f4','shared/ozon_contract.js':'42255016afbbb84bd5f930bd366d347f4e8e91adc76c56aac58818395dbce068','shared/ozon_entitlements.js':'65afa86fad24ad52070cfcdbfa820d14d8349ade585bfd568bc01139cbbdd109'}
...
def main():
 ...
 prev=v/'materialize_patch_b43_fbp_posting_reads_candidate.py';pg=v/'PATCH_B44_FBO_POSTING_GET_2026-08-28.patch.gz'
 c=pg.read_bytes();assert sha(c)==GZ;raw=gzip.decompress(c);assert sha(raw)==RAW
 if work.exists(): shutil.rmtree(work)
 work.mkdir(parents=True);base=work/'b43-base'
 if sys.platform.startswith('win'):
  prev_work=Path('C:/w44');prev_out=Path('C:/b43')
  for p in (prev_work,prev_out):
   if p.exists(): shutil.rmtree(p)
 else:
  prev_work=work/'b43-work';prev_out=base
 subprocess.run([sys.executable,str(prev),'--repo-root',str(repo),'--work-root',str(prev_work),'--out',str(prev_out)],check=True);assert tree(prev_out)==B43
 if prev_out!=base:
  if base.exists(): shutil.rmtree(base)
  shutil.copytree(prev_out,base);assert tree(base)==B43
 ...
 assert tree(out)==B44
 for m in ['PATCH_B44_B43_BASE_IDENTITY_PASS','PATCH_B44_PATCH_TRANSPORT_IDENTITY_PASS','PATCH_B44_PATCH_APPLY_PASS','PATCH_B44_PRODUCTION_FILE_COUNT_21_PASS','PATCH_B44_CHANGED_FILE_IDENTITIES_PASS','PATCH_B44_PROTECTED_B43_IDENTITIES_PASS','PATCH_B44_TREE_MANIFEST_SHA256_PASS']: print(m)
