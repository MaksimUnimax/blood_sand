...
DEV = OZON_ROOT / "development/alice-large-result-document-delivery-2026-09-10"
MATERIALIZER = DEV / "materialize_alice_large_result_patch.py"
HARDENER = DEV / "harden_scoped_alice_attachment.py"
...
def exact_diff_scope() -> None:
    ...
    if actual != expected:
        raise AssertionError(f"unexpected production/intersecting-regression diff scope: {sorted(actual)} != {sorted(expected)}")
    diff = run(["git", "diff", "--unified=0"], capture=True).stdout
    added = "\n".join(line[1:] for line in diff.splitlines() if line.startswith("+") and not line.startswith("+++"))
    for token in ("sellerApiKey", "performanceClientSecret", "Authorization", "Client-Id", "Api-Key",
...
def syntax_and_regressions() -> None:
    ...
    commands = [
        ["node", HERE / "run_alice_large_result_delivery_gate.mjs", DIST],
        ["node", HERE / "run_alice_large_result_delivery_gate_v2.mjs", REPO_ROOT, DIST],
...
def browser_and_mv3(chrome: Path) -> None:
    ...
    dumped = run([chrome, "--headless=new", "--no-sandbox", "--disable-gpu", "--disable-background-networking",
                  "--allow-file-access-from-files", "--dump-dom", fixture], capture=True).stdout
    # --dump-dom includes script source; do not search for the literal FAIL string in source code.
    if 'data-ozon-test="PASS"' not in dumped or '<pre id="result">ALICE_ATTACHMENT_BROWSER_FIXTURE_PASS</pre>' not in dumped:
...
def write_dependency_evidence() -> None:
    lines = ["# Alice large-result document delivery — dependency closure", "",
    ...
    lines += [f"| {n} | {d} | `{p}` | {b} | `{s}` |" for n,d,p,b,s in dependency_rows()]
    lines += ["", "Unaccounted dependencies: **0**.", "Stale assumptions after secondary sweep: **0**.",
              ...
              "**DEPENDENCY VERDICT: PENDING WINDOWS GATE**", "", "Live-only checks are not promoted to PASS by deterministic CI."]
...
def build_package() -> dict:
    ...
    buildinfo = [
        "patch=alice_large_result_document_delivery_2026-09-10", "version=0.1.19",
        f"package={ARTIFACT_NAME}", f"package_bytes={package_bytes}", f"package_sha256={package_sha}", f"production_files={len(files)}",
        "alice_plain_text_safe_threshold_utf16_code_units=90000", "alice_observed_live_composer_limit_reference=100000",
        "chatgpt_plain_text_threshold_unicode_code_points=1048000", "provider_calls_during_patch_gate=0",
        "automatic_retry_added=false", "automatic_pagination_added=false", "automatic_fanout_added=false",
        "windows_exact_source_package=PENDING_CI_JOB", "live_alice_dom=PENDING_POST_INSTALL", "live_large_result_delivery=PENDING_POST_INSTALL",
    ]
