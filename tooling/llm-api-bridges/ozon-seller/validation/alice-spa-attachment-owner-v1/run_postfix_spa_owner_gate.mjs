...
const wakePath = path.join(root, 'attachment_delivery_wake_content.js');
...
assert(content.includes('status(`Ozon: ошибка проверки активной доставки файла'), 'active recovery failure must remain user-visible');
assert(content.includes('idle attachment recovery probe unavailable:'), 'idle recovery failure must remain diagnostic without a user-visible plaque');
assert(!content.includes('status(`Ozon: ошибка проверки доставки файла ('), 'legacy unconditional recovery-error plaque must be absent');
assert(content.includes('async function recoverCurrent(expectation = null)'), 'recovery must accept explicit active-delivery expectation');
assert(wake.includes('runtime.recoverCurrent({') && wake.includes('delivery_id: String(message.delivery_id || "")'), 'explicit wake must pass active delivery expectation into recovery');
...
console.log('RECOVERY_ERROR_VISIBILITY=ACTIVE_ONLY_PASS');
console.log('IDLE_RECOVERY_FALSE_PLAQUE=ABSENT_PASS');
