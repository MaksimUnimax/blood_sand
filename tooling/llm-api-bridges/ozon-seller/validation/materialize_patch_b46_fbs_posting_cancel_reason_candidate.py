...
RAW='84a492a92852b3407ad5f109f0b3cae72305c9dda296b56ffcac74fd65df858c'
GZ='ad83581fb67c9aa57e29a32d05dca88e2b376dbcb9c8aa455882377281412873'
CH={'shared/ozon_operation_registry.js':'dc77665917457b8fe40463735611c78ec26698b6a53adda179fb98b93d9506d2','shared/ozon_contract.js':'256828877143dbc7b341405060b193c8a50b3275ea7c2484d02a2c0979141d68','shared/ozon_entitlements.js':'da6fc40e1dcd5118e9ab0ca1d62706d8a630648d17160155c14b368ae022614d'}
...
def main():
 ...
 prev=v/'materialize_patch_b45_seller_action_candidates_candidate.py';pg=v/'PATCH_B46_FBS_POSTING_CANCEL_REASON_2026-08-28.patch.gz'
 c=pg.read_bytes();assert sha(c)==GZ;raw=gzip.decompress(c);assert sha(raw)==RAW
 if work.exists(): shutil.rmtree(work)
 work.mkdir(parents=True);base=work/'b45-base'
 if sys.platform.startswith('win'):
  prev_work=Path('C:/w46');prev_out=Path('C:/b45')
  for p in (prev_work,prev_out):
   if p.exists(): shutil.rmtree(p)
 else:
  prev_work=work/'b45-work';prev_out=base
 subprocess.run([sys.executable,str(prev),'--repo-root',str(repo),'--work-root',str(prev_work),'--out',str(prev_out)],check=True);assert tree(prev_out)==B45
 if prev_out!=base:
  if base.exists(): shutil.rmtree(base)
  shutil.copytree(prev_out,base);assert tree(base)==B45
 ...
 assert tree(out)==B46
 for m in ['PATCH_B46_B45_BASE_IDENTITY_PASS','PATCH_B46_PATCH_TRANSPORT_IDENTITY_PASS','PATCH_B46_PATCH_APPLY_PASS','PATCH_B46_PRODUCTION_FILE_COUNT_21_PASS','PATCH_B46_CHANGED_FILE_IDENTITIES_PASS','PATCH_B46_PROTECTED_B45_IDENTITIES_PASS','PATCH_B46_TREE_MANIFEST_SHA256_PASS']: print(m)
