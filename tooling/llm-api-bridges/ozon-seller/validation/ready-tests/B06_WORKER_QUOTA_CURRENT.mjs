...
if (!candidateDir || !fs.existsSync(path.join(candidateDir, 'service_worker.js'))) {
  throw new Error('usage: node B06_WORKER_QUOTA_CURRENT.mjs <exact-current-candidate-dir>');
}
...
const chrome={
  runtime:{
    id:'current-worker-quota-test', lastError:null,
...
console.log('B06_MANUAL_PUBLIC_QUOTA_STATE_PASS');
...
console.log('B06_AUTORUN_PUBLIC_QUOTA_STATE_PASS');
console.log('B06_PUBLIC_QUOTA_PRIVACY_PASS');
...
// Durable guarded-wait restart scenario. 5s margin is intentional: unlike the rejected
// ~200ms seed, this gives the worker enough time to persist quota_waiting before due.
...
console.log('B06_DURABLE_QUOTA_WAIT_PASS');
console.log('B06_DUE_ONE_PROVIDER_CALL_PASS');
...
console.log('B06_ONE_429_ONE_PROVIDER_CALL_PASS');
console.log('B06_ZERO_IMMEDIATE_RETRY_PASS');
console.log('B06_ZERO_ALARM_REPLAY_PASS');
console.log('B06_ZERO_STARTUP_REPLAY_PASS');
console.log('B06_RETRY_AFTER_EXTENSION_ONLY_PASS');
...
console.log('B06_WORKER_QUOTA_CURRENT_PASS');
