...
assert(C&&typeof C.parseCommand==='function'&&typeof C.discoverCommands==='function','OzonContract API missing');
...
console.log('B02_VALID_ANALYTICS_ACCEPTED_PASS');

const discovered=C.discoverCommands(`Обычный текст до команды.\n\n\`\`\`text\n${commandText(validAnalytics)}\n\`\`\`\n\nОбычный Markdown после.`);
assert(Array.isArray(discovered)&&discovered.length===1,'surrounding Markdown created wrong command count');
assert(discovered[0]?.command?.operation==='analytics_data'||discovered[0]?.operation==='analytics_data','discovered command is not analytics_data');
console.log('B02_DISCOVERY_SURROUNDING_MARKDOWN_PASS');
...
const analyticsInvalids=[
  ...
  [{...validAnalytics,params:{...validAnalytics.params,metrics:Array.from({length:15},(_,i)=>i===0?'revenue':'ordered_units')}},'metric count over provider max'],
...
assert(!/api-seller\.ozon\.ru[^\n]*\$\{|new URL\([^)]*params|params\.(?:url|host|method|headers|auth)/i.test(source),'contract exposes assistant-controlled transport surface');
