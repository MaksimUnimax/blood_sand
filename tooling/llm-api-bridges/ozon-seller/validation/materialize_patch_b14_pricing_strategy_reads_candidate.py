...
RAW='0c98be2b82c800987b1a8c76e5ef86dcfe53ee5206e80d9fd9aac32e92f38ace'
GZ='b1f9839614e91372404928aa77200450fa120df52fd90fd36a326bfc5339f3ee'
COUNT=21
CHANGED={
...
def main():
 ...
 b13=v/'materialize_patch_b13_promotions_market_reads_candidate.py'; gz=v/'PATCH_B14_PRICING_STRATEGY_READS_2026-08-27.patch.gz'
 if not b13.is_file(): raise RuntimeError(f'missing B13 materializer: {b13}')
 if not gz.is_file(): raise RuntimeError(f'missing B14 patch: {gz}')
 comp=gz.read_bytes()
 if sha(comp)!=GZ: raise RuntimeError('B14 gzip identity mismatch')
 raw=gzip.decompress(comp)
 if sha(raw)!=RAW: raise RuntimeError('B14 raw patch identity mismatch')
 if work.exists(): shutil.rmtree(work)
 work.mkdir(parents=True); b13work=work/'b13-work'; b13out=work/'b13-base'
 subprocess.run([sys.executable,str(b13),'--repo-root',str(repo),'--work-root',str(b13work),'--out',str(b13out)],check=True)
 if tree(b13out)!=B13_TREE: raise RuntimeError('B13 base identity mismatch')
 if out.exists(): shutil.rmtree(out)
 shutil.copytree(b13out,out)
 ...
 if r.returncode: raise RuntimeError('B14 patch apply failed:\n'+r.stderr.decode(errors='replace'))
 files=[p for p in out.rglob('*') if p.is_file()]
 if len(files)!=COUNT: raise RuntimeError(f'B14 file count {len(files)} != {COUNT}')
 for rel,exp in CHANGED.items():
  got=sha((out/rel).read_bytes())
  if got!=exp: raise RuntimeError(f'B14 changed identity mismatch {rel}: {got} != {exp}')
 for rel,exp in PROTECTED.items():
  got=sha((out/rel).read_bytes())
  if got!=exp: raise RuntimeError(f'B14 protected B13 identity mismatch {rel}: {got} != {exp}')
 got=tree(out)
 if got!=B14_TREE: raise RuntimeError(f'B14 tree mismatch {got} != {B14_TREE}')
 for m in ['PATCH_B14_B13_BASE_IDENTITY_PASS','PATCH_B14_PATCH_TRANSPORT_IDENTITY_PASS','PATCH_B14_PATCH_APPLY_PASS','PATCH_B14_PRODUCTION_FILE_COUNT_21_PASS','PATCH_B14_CHANGED_FILE_IDENTITIES_PASS','PATCH_B14_PROTECTED_B13_IDENTITIES_PASS','PATCH_B14_TREE_MANIFEST_SHA256_PASS']: print(m)
 print(out)
