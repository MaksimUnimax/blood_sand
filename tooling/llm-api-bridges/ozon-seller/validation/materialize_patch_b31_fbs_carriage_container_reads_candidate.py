...
B30='4a50a0d40af544aa3006dcf1b378c04c70c555f92179364b12771d2ad1ae72ce';B31='8f371897d63a3c79bb11c8587db63fdb5335aa588f49f329d791b888d9ca55ae'
RAW='2a0cb527554feb6036ab04d23ead1f2bc4e60ee623e10378066545d48b091807';GZ='b045506e3b2180204e662b1b1dbe83e3fb94b469a1ad248d1ba59d6eb8a362e9'
CH={'shared/ozon_operation_registry.js':'758e8bae26ed2131a946b0fcfcdbeba6fe220ddf460997003937fbb8500d17a5','shared/ozon_contract.js':'57c4dc4005d1e1b27ad2b3f7a640c6e2d269a13166f99d93ff9b793169c211b4','shared/ozon_entitlements.js':'66742659063288b3eb507b0ca9a829aec30574f768ef288ddad5b7080100a984'}
...
def tree(r):
 ...
 return sha(s.encode())
def main():
 a=argparse.ArgumentParser();a.add_argument('--repo-root',required=True);a.add_argument('--work-root',required=True);a.add_argument('--out',required=True);x=a.parse_args();repo=Path(x.repo_root).resolve();work=Path(x.work_root).resolve();out=Path(x.out).resolve();v=repo/'tooling/llm-api-bridges/ozon-seller/validation';prev=v/'materialize_patch_b30_fbs_delivery_assembly_reads_candidate.py';pg=v/'PATCH_B31_FBS_CARRIAGE_CONTAINER_READS_2026-08-28.patch.gz';c=pg.read_bytes();assert sha(c)==GZ;raw=gzip.decompress(c);assert sha(raw)==RAW
 if work.exists():shutil.rmtree(work)
 work.mkdir(parents=True);base=work/'b30-base';subprocess.run([sys.executable,str(prev),'--repo-root',str(repo),'--work-root',str(work/'b30-work'),'--out',str(base)],check=True);assert tree(base)==B30
 ...
 assert tree(out)==B31
 for m in ['PATCH_B31_B30_BASE_IDENTITY_PASS','PATCH_B31_PATCH_TRANSPORT_IDENTITY_PASS','PATCH_B31_PATCH_APPLY_PASS','PATCH_B31_PRODUCTION_FILE_COUNT_21_PASS','PATCH_B31_CHANGED_FILE_IDENTITIES_PASS','PATCH_B31_PROTECTED_B30_IDENTITIES_PASS','PATCH_B31_TREE_MANIFEST_SHA256_PASS']:print(m)
