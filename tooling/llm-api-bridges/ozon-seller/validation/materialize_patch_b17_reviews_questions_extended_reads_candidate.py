...
RAW='0464197c4de3e2d6c66e2584b832f995d4847d225a1407cc4df0732bd841ec36'
GZ='4c51c31d9d7b82c513a420b253d6f0fcb149d742a5d3980eaea2d0fefa226d5d'
COUNT=21
CHANGED={
...
def main():
 ...
 b16=v/'materialize_patch_b16_warehouse_delivery_diagnostics_candidate.py'; gz=v/'PATCH_B17_REVIEWS_QUESTIONS_EXTENDED_READS_2026-08-27.patch.gz'
 if not b16.is_file(): raise RuntimeError(f'missing B16 materializer: {b16}')
 if not gz.is_file(): raise RuntimeError(f'missing B17 patch: {gz}')
 comp=gz.read_bytes()
 if sha(comp)!=GZ: raise RuntimeError('B17 gzip identity mismatch')
 raw=gzip.decompress(comp)
 if sha(raw)!=RAW: raise RuntimeError('B17 raw patch identity mismatch')
 if work.exists(): shutil.rmtree(work)
 work.mkdir(parents=True); b16work=work/'b16-work'; b16out=work/'b16-base'
 subprocess.run([sys.executable,str(b16),'--repo-root',str(repo),'--work-root',str(b16work),'--out',str(b16out)],check=True)
 if tree(b16out)!=B16_TREE: raise RuntimeError('B16 base identity mismatch')
 if out.exists(): shutil.rmtree(out)
 shutil.copytree(b16out,out)
 ...
 if r.returncode: raise RuntimeError('B17 patch apply failed:\n'+r.stderr.decode(errors='replace'))
 files=[p for p in out.rglob('*') if p.is_file()]
 if len(files)!=COUNT: raise RuntimeError(f'B17 file count {len(files)} != {COUNT}')
 for rel,exp in CHANGED.items():
  got=sha((out/rel).read_bytes())
  if got!=exp: raise RuntimeError(f'B17 changed identity mismatch {rel}: {got} != {exp}')
 for rel,exp in PROTECTED.items():
  got=sha((out/rel).read_bytes())
  if got!=exp: raise RuntimeError(f'B17 protected B16 identity mismatch {rel}: {got} != {exp}')
 got=tree(out)
 if got!=B17_TREE: raise RuntimeError(f'B17 tree mismatch {got} != {B17_TREE}')
 for m in ['PATCH_B17_B16_BASE_IDENTITY_PASS','PATCH_B17_PATCH_TRANSPORT_IDENTITY_PASS','PATCH_B17_PATCH_APPLY_PASS','PATCH_B17_PRODUCTION_FILE_COUNT_21_PASS','PATCH_B17_CHANGED_FILE_IDENTITIES_PASS','PATCH_B17_PROTECTED_B16_IDENTITIES_PASS','PATCH_B17_TREE_MANIFEST_SHA256_PASS']: print(m)
 print(out)
