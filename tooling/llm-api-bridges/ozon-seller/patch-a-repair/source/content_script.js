...
(() => {
  ...
  async function sendWorkSessionPrompt(messageText, intentId, revision) {
    ...
    if (!identity.conversation_id && sent.composer_empty === true) {
      ...
      const timer = setInterval(() => { void (async () => {
        ...
        if (Date.now() > deadline || currentIdentity.origin !== identity.origin || currentIdentity.ai_id !== identity.ai_id) return clearInterval(timer);
        if (!currentIdentity.conversation_id) return;
        const messages = assistantMessages(); const last = messages[messages.length - 1] || null;
    ...
    return { ok: true, sent: sent.composer_empty === true, identity, assistant_baseline_ids: assistantTurnIds() };
  }
  ...
  runtimeMessageListener = (message, _sender, sendResponse) => {
    ...
    if (message?.type === "OZ_WORK_RUNTIME_RENEW") {
      ...
      stopManualObserver();
      const currentKey = conversationKeyFromLocation();
      const applied = Boolean(message.visible === true && currentKey && currentKey === message.conversation_key);
      if (applied) applyManualMode(true, currentKey);
      sendResponse({ ok: true, runtime_generation: workRuntimeGeneration, ui_record_generation: `ui-${crypto.randomUUID()}`, identity: conversationIdentity(), applied });
      return false;
    }
