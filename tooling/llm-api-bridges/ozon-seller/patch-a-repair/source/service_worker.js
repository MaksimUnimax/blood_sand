/* global BB2ConversationIdentity, BB2ManualControls, OzonRuntime, OzonCredentials, OzonContract, OzonGuidance, OzonWorkSessionModel, BridgeAutorunModel, ProviderTransportCore, OzonProvider */
importScripts("shared/conversation_identity.js", "shared/runtime_names.js", "shared/work_session_model.js", "shared/manual_controls.js", "shared/ozon_credentials.js", "shared/ozon_contract.js", "shared/ozon_guidance.js", "shared/bridge_autorun_model.js", "shared/provider_transport_core.js", "shared/ozon_provider.js");
...
async function acquireAnalyticsProviderQuota(rawCredentials, nowMs = Date.now()) {
  const identity = await sellerQuotaIdentity(rawCredentials);
...
async function mutateWorkSession(key, expectedRevision, nextState, patch = {}) {
  return withBindingWrite(async () => {
    const sessions = await getWorkSessions(); const current = OzonWorkSessionModel.normalize(sessions[key], key);
    if (expectedRevision !== null && Number(expectedRevision) !== current.revision) throw Object.assign(new Error("Устаревшее событие work-session проигнорировано."), { code: "WORK_SESSION_STALE_EVENT" });
    const next = OzonWorkSessionModel.transition(current, nextState, { ...patch, conversation_key: key }); sessions[key] = next; await storageSet({ [KEYS.WORK_SESSIONS]: sessions }); return next;
  });
}
...
async function createPendingWorkStart(tab, identity) {
  const pending = await getPendingWorkStarts(); if (pending[String(tab)]) return { duplicate: true, transaction: pending[String(tab)] };
  const transaction = { version: 1, state: OzonWorkSessionModel.STATES.PENDING_IDENTITY, intent_id: `work-start-${crypto.randomUUID()}`, revision: 1, tab_id: tab, origin: identity.origin, ai_id: identity.ai_id, created_at: new Date().toISOString(), expires_at: new Date(Date.now() + 120000).toISOString(), prompt_delivered: false, observed_conversation_id: null, first_response_complete: false };
  pending[String(tab)] = transaction; await storageSet({ [KEYS.PENDING_WORK_STARTS]: pending }); return { duplicate: false, transaction };
}
async function terminalizeRefreshOperation(key) {
  const operation = await getManualOperation(key); if (!operation || !manualOperationActive(operation)) return { operation_id: null, phase: "none", provider_dispatched: false };
  const phase = String(operation.status || operation?.batch?.request_state || "unknown");
  const requesting = phase === MANUAL_OPERATION_STATUSES.REQUESTING || operation?.batch?.request_state === "requesting";
  const code = requesting ? "REQUEST_OUTCOME_UNKNOWN_NO_RETRY" : "OPERATOR_REFRESH_BEFORE_PROVIDER";
  await mutateManualOperation(key, (current) => current && current.operation_id === operation.operation_id ? { ...current, status: "error", last_error: { code, message: requesting ? "Refresh во время provider request: исход неизвестен, повтор запрещён." : "Операция отменена операторским Refresh до provider dispatch." }, batch: current.batch ? { ...current.batch, request_state: requesting ? "outcome_unknown" : "terminal" } : current.batch } : current);
  return { operation_id: operation.operation_id, phase, provider_dispatched: requesting };
}
async function beginWorkSessionRefresh(tab, key) {
  const session = await workSessionFor(key); if (![OzonWorkSessionModel.STATES.ACTIVE_VISIBLE, OzonWorkSessionModel.STATES.ACTIVE_HIDDEN].includes(session.state)) throw Object.assign(new Error("Refresh доступен только для активной work-session."), { code: "WORK_SESSION_NOT_ACTIVE" });
  const recoveries = await getWorkRecoveries(); if (recoveries[key]) return { already_in_progress: true, recovery: recoveries[key] };
  const live = await assertTabConversation(tab, key); const operation = await terminalizeRefreshOperation(key);
  const recovering = await mutateWorkSession(key, session.revision, OzonWorkSessionModel.STATES.RECOVERING);
  const recovery = { version: 1, recovery_id: `work-recovery-${crypto.randomUUID()}`, revision: recovering.revision, old_runtime_generation: WORKER_SESSION_ID, tab_id: tab, origin: live.origin, ai_id: live.ai_id, conversation_id: live.conversation_id, conversation_key: key, previous_state: session.state, operation, created_at: new Date().toISOString(), expires_at: new Date(Date.now() + 120000).toISOString(), expected_visible: session.state === OzonWorkSessionModel.STATES.ACTIVE_VISIBLE };
  recoveries[key] = recovery; await storageSet({ [KEYS.WORK_SESSION_RECOVERIES]: recoveries }); await diagnostic("WORK_SESSION_REFRESH_PREPARED", { recovery_id: recovery.recovery_id, old_runtime_generation: WORKER_SESSION_ID, operation_phase: operation.phase, provider_dispatched: operation.provider_dispatched });
  return { already_in_progress: false, recovery };
}
async function resumeWorkSessionRecoveries() {
  const recoveries = await getWorkRecoveries(); for (const [key, recovery] of Object.entries(recoveries)) {
    if (!recovery || recovery.expires_at < new Date().toISOString()) { delete recoveries[key]; continue; }
    const session = await workSessionFor(key); if (session.state !== OzonWorkSessionModel.STATES.RECOVERING || Number(session.revision) !== Number(recovery.revision)) continue;
    const identity = await tabIdentity(Number(recovery.tab_id)); if (!identity || identity.origin !== recovery.origin || identity.ai_id !== recovery.ai_id || identity.conversation_id !== recovery.conversation_id) { await mutateWorkSession(key, session.revision, OzonWorkSessionModel.STATES.ERROR, { error: { code: "WORK_REFRESH_CONTEXT_INVALID" } }); delete recoveries[key]; continue; }
    const target = recovery.expected_visible ? OzonWorkSessionModel.STATES.ACTIVE_VISIBLE : OzonWorkSessionModel.STATES.ACTIVE_HIDDEN; const response = await tabMessage(Number(recovery.tab_id), { type: "OZ_WORK_RUNTIME_RENEW", runtime_generation: WORKER_SESSION_ID, conversation_key: key, visible: recovery.expected_visible }); if (!response?.ok || response.runtime_generation !== WORKER_SESSION_ID) continue;
    await mutateWorkSession(key, session.revision, target); if (target === OzonWorkSessionModel.STATES.ACTIVE_VISIBLE) await setManualMode(key, true); delete recoveries[key]; await diagnostic("WORK_SESSION_REFRESH_RESUMED", { recovery_id: recovery.recovery_id, old_runtime_generation: recovery.old_runtime_generation, new_runtime_generation: WORKER_SESSION_ID });
...
async function bindConversation(context) {
  ...
  return withBindingWrite(async () => {
    ...
    if (currentSession.state === OzonWorkSessionModel.STATES.INACTIVE || currentSession.state === OzonWorkSessionModel.STATES.ERROR) {
      await mutateWorkSession(key, currentSession.revision, OzonWorkSessionModel.STATES.BINDING, { tab_id: tab, origin: live.origin, ai_id: live.ai_id, conversation_id: live.conversation_id });
    }
    return record;
  });
}
...
async function getSettings() {
  const data = await storageGet([
    ...
    KEYS.AUTO_SEND, KEYS.LAST_STATUS
  ]);
  return {
...
async function executeManualCommand(commandText, conversationKey, sender, manualRequestId) {
  ...
  if (!(await getManualMode(key))) {
    ...
    entries = [batchErrorEntry(Object.assign(new Error("Ручной режим Ozon выключен для этого AI-диалога. API-запрос не выполнен."), { code: "MANUAL_MODE_OFF" }), sourceStage, OzonContract.textFingerprint(commandText))];
  } else {
    const run = await getAutoRun(key);
    if (run && !BridgeAutorunModel.canEnableManualMode(run.status)) {
      sourceStage = "manual_gate";
...
function discoverBatchEntries(text) {
  ...
  const helpCount = source.split(OzonRuntime.RUNTIME.helpPrefix).length - 1;
  if (apiCount && helpCount) return [batchErrorEntry(Object.assign(new Error("OZON_HELP_V1 нельзя смешивать с OZON_API_V1 в одном assistant response."), { code: "MIXED_HELP_AND_API" }), "guidance_discovery", OzonContract.textFingerprint(source))];
  if (helpCount) {
    const help = OzonGuidance.parseHelp(source);
    if (!help.ok) return [{ kind: "guidance", status: "pending", guidance: { status: "guidance_error", cluster: null, error: help.code }, request_id: null, http_status: 0, external_request_executed: false, report_text: null }];
    return [{ kind: "guidance", status: "pending", guidance: { status: "cluster_selected", cluster: help.cluster, error: null }, request_id: null, http_status: 0, external_request_executed: false, report_text: null }];
  }
...
function localGuidanceResult(entry) {
  ...
  const payload = OzonGuidance.result({ status, cluster, error: error.code || classified.error || null, descriptor });
  return { request_id: `guidance-${crypto.randomUUID()}`, report_text: OzonGuidance.format(payload), status, cluster, external_request_executed: false };
}
...
async function ensureBatchCapabilityAndPlanning({ ownerKind, ownerId, getOwner, mutateOwner, ownerMatches, isCollecting, failOwner }) {
  ...
  const capabilityRequired = commandEntries.some((entry) => {
    try { return OzonContract.sellerCapabilityRequirement(entry.command, planningAtMs).required === true; }
  ...
  if (!capabilityRequired) {
    ...
    if (!resolution || !["complete", "not_needed"].includes(String(resolution.state || ""))) {
      ...
      if (probeGranted) {
        ...
        const settings = await getSettings();
        const profile = await OzonProvider.resolveSellerCapability(settings.sellerCredentials);
        let stored = false;
        owner = await mutateOwner((current) => {
          if (!current || !ownerMatches(current) || !isCollecting(current) || !current.batch) return current;
          const live = current.batch.capability_resolution;
...
function processBatchQueue({
  ...
  return singleFlight(batchCollectionRequests, flightKey, async () => {
    ...
    // Guidance is a local parser result. It must not enter capability planning,
    // quota/cache scheduling, or any provider code path.
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  (async () => {
    switch (message?.type) {
      ...
      case "OZ_WORK_SHOW": {
        const key = normalizeConversationKey(message.conversation_key); const session = await workSessionFor(key);
        ...
        const live = await assertTabConversation(normalizeTabId(message.tab_id), key); await strictBindingForIdentity(live);
        const next = await mutateWorkSession(key, session.revision, OzonWorkSessionModel.STATES.ACTIVE_VISIBLE);
        await setManualMode(key, true); await tabMessage(normalizeTabId(message.tab_id), { type: "OZ_APPLY_MANUAL_MODE", enabled: true, conversation_key: key });
        return { ok: true, session: next, state: await publicSettingsState(key) };
      }
      case "OZ_WORK_REFRESH": {
        const tab = normalizeTabId(message.tab_id); const key = normalizeConversationKey(message.conversation_key); const begun = await beginWorkSessionRefresh(tab, key);
        if (begun.already_in_progress) return { ok: true, code: "REFRESH_ALREADY_IN_PROGRESS", recovery: begun.recovery };
        // This is intentionally a true MV3 lifecycle renewal, not a UI repaint.
        setTimeout(() => chrome.runtime.reload(), 0);
        return { ok: true, recovery: begun.recovery, runtime_reload_scheduled: true };
      }
      case "OZ_WORK_START": {
        const tab = normalizeTabId(message.tab_id); const live = await tabIdentity(tab);
        if (!live.conversation_id) {
          ...
          if (!sent?.ok || sent.sent !== true) { const starts = await getPendingWorkStarts(); delete starts[String(tab)]; await storageSet({ [KEYS.PENDING_WORK_STARTS]: starts }); return { ok: false, code: sent?.code || "WORK_START_SEND_FAILED", error: sent?.error || "Initial prompt не отправлен." }; }
          const starts = await getPendingWorkStarts(); if (starts[String(tab)]?.intent_id === pending.transaction.intent_id) { starts[String(tab)] = { ...starts[String(tab)], prompt_delivered: true }; await storageSet({ [KEYS.PENDING_WORK_STARTS]: starts }); }
          return { ok: true, accepted: true, pending_start: { ...pending.transaction, prompt_delivered: true } };
        }
        const binding = await bindConversation({ tab_id: tab, origin: live.origin, conversation_id: live.conversation_id }); const key = binding.conversation_key;
        const session = await workSessionFor(key); const visible = await mutateWorkSession(key, session.revision, OzonWorkSessionModel.STATES.ACTIVE_VISIBLE, { tab_id: tab, origin: live.origin, ai_id: live.ai_id, conversation_id: live.conversation_id, start_intent_id: String(message.start_intent_id || crypto.randomUUID()) });
        await setManualMode(key, true); await tabMessage(tab, { type: "OZ_APPLY_MANUAL_MODE", enabled: true, conversation_key: key });
        const prompt = await getAutoStartPrompt(key); const sent = await tabMessage(tab, { type: "OZ_WORK_SEND_INITIAL_PROMPT", intent_id: visible.start_intent_id, revision: visible.revision, prompt_text: String(prompt?.text || DEFAULT_AUTO_START_TEXT) });
        if (!sent?.ok || sent.sent !== true) { await setManualMode(key, false); await mutateWorkSession(key, visible.revision, OzonWorkSessionModel.STATES.ERROR, { error: { code: sent?.code || "WORK_START_SEND_FAILED" } }); return { ok: false, code: sent?.code || "WORK_START_SEND_FAILED", error: sent?.error || "Initial prompt не отправлен." }; }
        return { ok: true, binding, session: visible, state: await publicSettingsState(key) };
      }
      case "OZ_WORK_PENDING_IDENTITY": {
        const tab = normalizeTabId(sender?.tab?.id || message.tab_id); const starts = await getPendingWorkStarts(); const pending = starts[String(tab)]; const identity = normalizeIdentity(message.identity || {});
        if (!pending || pending.intent_id !== String(message.intent_id || "") || Number(pending.revision) !== Number(message.revision) || pending.origin !== identity.origin || pending.ai_id !== identity.ai_id || !identity.conversation_id || pending.expires_at < new Date().toISOString()) return { ok: false, code: "WORK_PENDING_STALE_OR_INVALID" };
        if (message.first_response_complete !== true) { starts[String(tab)] = { ...pending, observed_conversation_id: identity.conversation_id }; await storageSet({ [KEYS.PENDING_WORK_STARTS]: starts }); return { ok: true, waiting: true }; }
        const binding = await bindConversation({ tab_id: tab, origin: identity.origin, conversation_id: identity.conversation_id }); const key = binding.conversation_key; const base = await workSessionFor(key); const bindingState = base.state === OzonWorkSessionModel.STATES.INACTIVE ? await mutateWorkSession(key, base.revision, OzonWorkSessionModel.STATES.BINDING, { tab_id: tab, origin: identity.origin, ai_id: identity.ai_id, conversation_id: identity.conversation_id, start_intent_id: pending.intent_id }) : base; const visible = await mutateWorkSession(key, bindingState.revision, OzonWorkSessionModel.STATES.ACTIVE_VISIBLE); await setManualMode(key, true); await tabMessage(tab, { type: "OZ_APPLY_MANUAL_MODE", enabled: true, conversation_key: key }); delete starts[String(tab)]; await storageSet({ [KEYS.PENDING_WORK_STARTS]: starts }); return { ok: true, binding, session: visible };
      }
      case "OZ_WORK_HIDE": {
        const key = normalizeConversationKey(message.conversation_key); const session = await workSessionFor(key);
        ...
        await setManualMode(key, false); await tabMessage(normalizeTabId(message.tab_id), { type: "OZ_APPLY_MANUAL_MODE", enabled: false, conversation_key: key });
        return { ok: true, session: next, state: await publicSettingsState(key) };
      }
      case "OZ_WORK_FINISH": {
        const key = normalizeConversationKey(message.conversation_key); const session = await workSessionFor(key);
        ...
        await setManualMode(key, false); await tabMessage(normalizeTabId(message.tab_id), { type: "OZ_APPLY_MANUAL_MODE", enabled: false, conversation_key: key });
        await stopAutoRun(key); const sessions = await getWorkSessions(); const bindings = await getConversationBindings(); delete bindings[key]; sessions[key] = await retireWorkSession(key); await storageSet({ [KEYS.CONVERSATION_BINDINGS]: bindings, [KEYS.WORK_SESSIONS]: sessions });
        return { ok: true, state: await publicSettingsState(key) };
      }
      ...
      case "OZ_GET_MANUAL_STATE": {
        ...
        const [enabled, operation] = await Promise.all([getManualMode(key), getManualOperation(key)]);
        const active = manualOperationActive(operation);
        return { ok: true, enabled, ready: enabled === true && !active, bound: true, manual_operation_active: active, manual_operation: publicManualOperation(operation) };
      }
