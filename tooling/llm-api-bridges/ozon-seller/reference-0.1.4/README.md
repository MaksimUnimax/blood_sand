...
`run_tests.sh` reconstructs the v0.1.4 production tree from the sibling canonical `reference-0.1.3/archive-exact/` artifact, applies `evidence/OZON_BRIDGE_V0.1.4_PATCH.diff`, verifies SHA-256 for all 16 production files, then runs the 32-test Node suite with coverage, including VM execution of the actual `reportAutoPreExecutionError()` function extracted from the packaged content script.
