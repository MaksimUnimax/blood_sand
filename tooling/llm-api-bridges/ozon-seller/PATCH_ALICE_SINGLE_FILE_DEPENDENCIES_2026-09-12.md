...
| P24 | Тестовые assumptions и package | old IDB call-site audit updated 3/3→4/5; new audit/run → git blobs → deterministic ZIP → all phases | Only test assertion counts expanded; negative writer tests preserved | 10 mutants, extra-file mutation, all31 scripts, final same ZIP SHA across platforms |
