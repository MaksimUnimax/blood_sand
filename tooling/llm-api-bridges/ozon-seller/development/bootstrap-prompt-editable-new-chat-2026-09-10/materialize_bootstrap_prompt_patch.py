...
def patch_runtime_names() -> None:
    ...
    if 'GLOBAL_AUTO_START_PROMPT: "ozmb_global_auto_start_prompt_v1"' not in text:
...
async function getAutoStartPrompt(conversationKey, { ensureStored = true } = {}) {
  ...
  return withStartPromptWrite(async () => {
    ...
    // Legacy is_default=true records were stored copies of the built-in default.
    // They are not user overrides and must inherit the editable global bootstrap prompt.
...
'''
    ...
    old_global_state = '    auto_start_prompt: { text: DEFAULT_AUTO_START_TEXT, is_default: true, updated_at: null },'
    new_global_state = '    auto_start_prompt: { ...common.global_auto_start_prompt, is_override: false, source: "global" },'
    text = replace_once(text, old_global_state, new_global_state, "new-chat effective prompt state")
...
def patch_popup_js() -> None:
    ...
    old_common = '    auto_send: $("autoSend").checked,\n    personal_data_enabled: $("personalDataEnabled")?.checked === true\n'
    new_common = '    auto_send: $("autoSend").checked,\n    personal_data_enabled: $("personalDataEnabled")?.checked === true,\n    global_auto_start_prompt_text: $("globalAutoStartPromptText").value\n'
    text = replace_once(text, old_common, new_common, "popup save global prompt regardless of identity")
    ...
    old_status = '    status("Стартовый prompt возвращён к безопасному значению по умолчанию.", "ok");'
    new_status = '    status("Индивидуальный prompt удалён: текущий диалог снова использует общий стартовый prompt.", "ok");'
    text = replace_once(text, old_status, new_status, "current prompt reset semantics")
...
def main() -> None:
    ...
    checks = [
        ...
        ('source: "conversation_override"' in worker, "per-conversation override preserved"),
        ('source: "global"' in worker, "global inheritance"),
        ('const bootstrapPrompt = await getGlobalAutoStartPrompt();' in worker, "new chat bootstrap read"),
        ('prompt_text: DEFAULT_AUTO_START_TEXT' not in worker, "no hard-coded new-chat send"),
    ...
    print("BOOTSTRAP_PROMPT_MATERIALIZATION_PASS")
