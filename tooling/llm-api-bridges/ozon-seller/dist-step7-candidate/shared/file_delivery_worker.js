...
(() => {
  ...
  function safeFilenameForResponse(url, headers, contentType, fileRef = "") {
    ...
    if (fromDisposition) return sanitizeFilename(fromDisposition, `ozon-report-${fileRef || "file"}.${extensionForContentType(contentType)}`);
    try {
      const pathname = new URL(String(url || "")).pathname;
      const base = decodeURIComponent(pathname.split("/").filter(Boolean).pop() || "");
      if (base && /\.[A-Za-z0-9]{1,10}$/.test(base)) return sanitizeFilename(base);
    } catch (_) {}
    return sanitizeFilename(`ozon-report-${fileRef || "file"}.${extensionForContentType(contentType)}`);
  }
  ...
  async function reportRefsForTrustedUrl(trustedUrl) {
    ...
    for (const [ref, record] of Object.entries(state?.report_file_refs || {})) {
      if (!record || typeof record !== "object" || typeof record.url !== "string") continue;
      try {
  ...
  async function findAttachmentOwner(conversationKey, preferredKind = "", preferredId = "") {
    ...
    for (const kind of kinds) {
      ...
      if (owner?.status && owner?.delivery?.mode === ATTACHMENT_MODE) return { kind, owner };
    }
    return null;
  }
  ...
  async function materializeInlineProviderArtifact(fileRef) {
    ...
    const filename = sanitizeFilename(`ozon-report-${fileRef}.${extension}`);
    const sha = await sha256Hex(bytes);
    const created = nowMs();
    const artifact = {
      artifact_key: `provider:${fileRef}`,
  ...
  async function ensureArtifactsForDelivery(delivery) {
    ...
    for (const ref of Array.isArray(delivery?.provider_file_refs) ? delivery.provider_file_refs : []) {
      ...
      if (!artifact) {
        throw Object.assign(new Error("Original Ozon report bytes are unavailable for attachment; automatic re-download is forbidden."), { code: "REPORT_FILE_ARTIFACT_NOT_CAPTURED" });
      }
  ...
  function adapterCanAttach(delivery, records) {
    ...
    for (const record of records) {
      ...
      if (support.supported !== true) {
        throw Object.assign(new Error(`Target AI does not support original attachment type .${record.extension || "unknown"}.`), { code: "TARGET_AI_FILE_TYPE_UNSUPPORTED" });
      }
    }
    return true;
  }
  ...
  async function ownerForMessage(message, sender) {
    ...
    if (message.delivery_id && String(found.owner.delivery?.delivery_id || "") !== String(message.delivery_id)) {
      throw Object.assign(new Error("Attachment delivery ID mismatch."), { code: "ATTACHMENT_DELIVERY_ID_MISMATCH" });
    }
    return { key, ...found };
  }
  ...
  async function commitAttachmentSend(message, sender) {
    ...
    const next = await mutateOwner(found.key, found.kind, ownerId(found.owner, found.kind), (current) => {
      ...
      return {
        ...
        delivery: {
          ...current.delivery,
          phase: PHASES.SEND_COMMITTED,
          attachment_send_actor_id: actorId || null,
          attachment_send_committed_at: nowIso(),
          baseline_user_turn_ids: Array.isArray(message.baseline_user_turn_ids) ? message.baseline_user_turn_ids.map(String).slice(0, 5000) : []
        }
      };
    });
  ...
  async function confirmAttachmentDelivery(message, sender) {
    ...
    const next = await mutateOwner(found.key, found.kind, ownerId(found.owner, found.kind), (current) => {
      ...
      if (found.kind === "manual") {
        return {
          ...
          last_confirmed_user_turn_id: confirmedUserTurnId,
          batch: null,
          delivery: null,
          outgoing_text: null,
    ...
    if (found.kind === "autorun" && next?.status === BridgeAutorunModel.RUN_STATUSES.WAITING_COMMAND) {
      try {
        await chrome.tabs.sendMessage(Number(next.tab_id), {
          ...
          assistant_baseline_ids: Array.isArray(next.assistant_baseline_ids) ? next.assistant_baseline_ids : []
        });
      } catch (_) {}
    }
    if (found.kind === "manual") {
  ...
  async function pushCurrentAttachmentDeliveries() {
    const data = await chrome.storage.local.get([KEYS.AUTO_RUNS, KEYS.MANUAL_OPERATIONS]);
    for (const owner of Object.values(data?.[KEYS.AUTO_RUNS] || {})) if (owner?.delivery?.mode === ATTACHMENT_MODE) await pushOwner("autorun", owner);
    for (const [key, owner] of Object.entries(data?.[KEYS.MANUAL_OPERATIONS] || {})) {
      if (await cancelManualClaimedIfDisabled(key, owner)) continue;
      if (owner?.delivery?.mode === ATTACHMENT_MODE) await pushOwner("manual", owner);
    }
  }

  chrome.storage.onChanged.addListener((changes, areaName) => {
    if (areaName !== "local") return;
    if (!changes[KEYS.AUTO_RUNS] && !changes[KEYS.MANUAL_OPERATIONS] && !changes[KEYS.MANUAL_MODES]) return;
    queueMicrotask(() => { void pushCurrentAttachmentDeliveries(); });
  });

  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (!String(message?.type || "").startsWith("OZ_ATTACHMENT_")) return false;
    (async () => {
      switch (message.type) {
        ...
        default: return { ok: false, code: "ATTACHMENT_MESSAGE_UNSUPPORTED" };
      }
    })().then(sendResponse).catch((error) => sendResponse({ ok: false, code: error?.code || "ATTACHMENT_WORKER_ERROR", error: String(error?.message || error) }));
    return true;
  });

  setTimeout(() => { void cleanupExpiredArtifacts(); void pushCurrentAttachmentDeliveries(); }, 0);
