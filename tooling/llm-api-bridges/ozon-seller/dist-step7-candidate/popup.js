...
async function resolvePopupContext({ required = true } = {}) {
  ...
  if (!resolved.ok || !resolved.context?.conversation_key) {
    const supportedNewChat = ["chatgpt", "alice"].includes(String(page.identity?.ai_id || "")) && !page.identity?.conversation_id;
    const error = supportedNewChat
      ? 'Новый AI-чат ещё не имеет устойчивого conversation ID. Нажмите «Начать работу»: Bridge сам отправит начальный prompt и привяжет появившийся диалог после завершения первого ответа.'
...
function renderState(state) {
  ...
  const session = state.work_session || { state: "inactive" };
  ...
  if ($("personalDataEnabled")) $("personalDataEnabled").checked = state.personal_data_enabled === true;
  if ($("personalDataMeta")) $("personalDataMeta").textContent = state.personal_data_enabled === true
    ? "Включено: personal-data READ может быть отправлен в Ozon и доставлен только в привязанный AI-диалог. Выключение действует на новые запросы; автоматического replay нет."
    : "Выключено: personal-data READ завершается локально, Ozon не вызывается, AI просит оператора включить разрешение и затем явно повторить команду.";
  ...
  $("providerGateMeta").textContent = state.provider_execution_ready
    ? `READ operations готовы: ${enabledOps.join(", ") || "—"}. Personal-data READ технически поддерживаются и выполняются только по политике оператора.`
    : "Execution gate CLOSED: нет исполняемых READ operations.";

  const metadata = state.seller_api_metadata || {};
  if ($("sellerMetadataSource")) $("sellerMetadataSource").textContent = metadata.source_kind === "remote_official" ? "Ozon Swagger" : "Bundled LKG";
  if ($("sellerMetadataOperations")) $("sellerMetadataOperations").textContent = Number.isFinite(Number(metadata.operation_count)) ? String(Number(metadata.operation_count)) : "—";
  if ($("sellerMetadataRules")) $("sellerMetadataRules").textContent = Number.isFinite(Number(metadata.entitlement_rule_count)) ? String(Number(metadata.entitlement_rule_count)) : "—";
  if ($("sellerMetadataUnresolved")) $("sellerMetadataUnresolved").textContent = Number.isFinite(Number(metadata.unresolved_rule_count)) ? String(Number(metadata.unresolved_rule_count)) : "—";
  if ($("sellerMetadataMeta")) {
    const captured = metadata.captured_at ? ` · ${metadata.captured_at}` : "";
    const hash = metadata.source_hash ? ` · SHA ${String(metadata.source_hash).slice(0, 12)}…` : "";
  ...
  if ($("manualModeMeta")) {
    if (!pageAvailable) $("manualModeMeta").textContent = "Кнопка Ozon недоступна: текущий AI-диалог ещё не имеет подтверждённой identity. Для нового чата используйте «Начать работу» выше.";
    else if (session.state === "active_visible") $("manualModeMeta").textContent = manualOperationActive ? "Кнопка Ozon включена; текущая операция ещё выполняется/доставляется." : "Кнопка Ozon включена для текущей work-session.";
    else if (session.state === "active_hidden") $("manualModeMeta").textContent = "Кнопка Ozon скрыта; binding и Ozon provider state сохранены.";
    else $("manualModeMeta").textContent = "Кнопка Ozon выключена. Управление выполняется только через Start / Show / Hide / Refresh / Finish выше.";
  }
}

$("workStart")?.addEventListener("click", () => busy($("workStart"), async () => {
  const context = await resolvePopupContext({ required: false });
  const identity = context.page?.identity || null;
  if (!context.tab_id || !identity || !["chatgpt", "alice"].includes(String(identity.ai_id || ""))) {
    return status("Текущая вкладка не является поддерживаемым ChatGPT / Alice диалогом.", "error");
  }
  const response = await send("OZ_WORK_START", { tab_id: context.tab_id, start_intent_id: crypto.randomUUID() });
  if (!response.ok) return status(`${response.code || "ERROR"}: ${response.error || "Не удалось начать работу."}`, "error");
  if (response.code === "WORK_START_ALREADY_PENDING") return status("Start уже выполняется для этой вкладки; повторный prompt не отправлен.", "ok");
  await refresh();
  status(response.pending_start ? "Начальный prompt отправлен. Жду настоящий conversation ID и завершение первого ответа." : "Work-session активирована; кнопка Ozon включена для текущего диалога.", "ok");
}));
$("workRefresh")?.addEventListener("click", () => busy($("workRefresh"), async () => {
  const context = await resolvePopupContext(); const response = await send("OZ_WORK_REFRESH", { tab_id: context.tab_id, conversation_key: context.conversation_key });
  if (!response.ok) return status(response.error || "Не удалось подготовить Refresh.", "error");
...
async function refresh() {
  ...
  const state = {
    ...
    work_start_available: Boolean(context.tab_id && context.page?.identity && ["chatgpt", "alice"].includes(String(context.page.identity.ai_id || "")))
  };
  renderState(state);
...
$("personalDataEnabled")?.addEventListener("change", async () => {
  try {
    const enabled = $("personalDataEnabled").checked === true;
    await saveAll({ includeKey: false });
    status(enabled
      ? "«Показывать личные данные» включено. Ранее заблокированные команды не replay-ятся: отправьте нужную команду заново."
      : "«Показывать личные данные» выключено. Новые personal-data READ будут блокироваться локально без запроса к Ozon.", "ok");
  } catch (error) {
    await refresh();
...
$("refreshSellerMetadata")?.addEventListener("click", () => busy($("refreshSellerMetadata"), async () => {
  const response = await send("OZ_REFRESH_SELLER_API_METADATA", {});
  await refresh();
  if (!response.ok) {
    return status(`${response.code || "SELLER_METADATA_UPDATE_FAILED"}: ${response.error || "Не удалось обновить правила Ozon API."} Предыдущий проверенный snapshot сохранён.`, "error");
  }
  const summary = response.summary || {};
  status(`Правила Ozon API обновлены: ${summary.operation_count || 0} операций, Premium-правил ${summary.entitlement_rule_count || 0}, не разобрано ${summary.unresolved_rule_count || 0}. Изменения: +${summary.added || 0} / -${summary.removed || 0} / ~${summary.changed || 0}; entitlement ~${summary.entitlement_changed || 0}.`, "ok");
}));
