...
(() => {
  ...
  const RUNTIME_KEY = "__OZON_ATTACHMENT_DELIVERY_RUNTIME__";
  const runtime = { id: `attachment:${Date.now()}:${Math.random().toString(36).slice(2)}`, disposed: false, in_flight: new Set() };
  const previous = globalThis[RUNTIME_KEY];
  if (previous?.dispose) { try { previous.dispose(); } catch (_) {} }
  ...
  function sameConversation(recovery) {
    const here = identity();
    return Boolean(
      here.status === "confirmed" && here.conversation_id &&
    ...
    return new Promise((resolve) => {
      try {
        chrome.runtime.sendMessage({ type, ...payload }, (response) => {
          ...
          if (error) return resolve({ ok: false, code: "RUNTIME_ERROR", error: error.message });
          resolve(response || { ok: false, code: "EMPTY_RESPONSE", error: "Empty worker response." });
        });
      } catch (error) {
  ...
  function status(message, tone = "info", timeout = 7000) {
    ...
    if (!(root instanceof HTMLElement)) {
      ...
      Object.assign(root.style, {
        position: "fixed", right: "18px", top: "82px", zIndex: "2147483646",
        maxWidth: "460px", whiteSpace: "pre-wrap", padding: "10px 12px", borderRadius: "10px",
        font: "13px/1.4 system-ui, sans-serif", boxShadow: "0 8px 24px rgba(15,23,42,.18)", pointerEvents: "none"
      });
  ...
  function composerContext() {
    const active = adapter();
    return active?.composerContext?.() || null;
  }
  ...
  async function waitForComposerAvailable(recovery) {
    while (current() && sameConversation(recovery)) {
      ...
      const check = await sendRuntime("OZ_ATTACHMENT_RECOVERY_GET", { conversation_key: recovery.conversation_key, ...ownerPayload(recovery) });
  ...
  async function fetchArtifactBytes(recovery, descriptor) {
    ...
    let offset = 0;
    while (offset < total) {
      const response = await sendRuntime("OZ_ATTACHMENT_ARTIFACT_CHUNK", {
        ...ownerPayload(recovery),
        artifact_key: descriptor.artifact_key,
        offset,
        length: Math.min(ARTIFACT_CHUNK_BYTES, total - offset)
      });
    ...
    if (sha !== String(descriptor.sha256 || "").toLowerCase()) throw Object.assign(new Error("Complete attachment SHA-256 mismatch."), { code: "ATTACHMENT_SHA256_MISMATCH" });
    return bytes;
  }

  async function buildFiles(recovery) {
    const metadata = await sendRuntime("OZ_ATTACHMENT_ARTIFACT_META", ownerPayload(recovery));
  ...
  async function failDelivery(recovery, error) {
    const response = await sendRuntime("OZ_ATTACHMENT_FAIL", {
      ...ownerPayload(recovery),
      code: String(error?.code || "ATTACHMENT_DELIVERY_FAILED"),
      error: String(error?.message || error || "Attachment delivery failed.")
    });
  ...
  async function reconcileCommittedAttachment(recovery) {
    ...
    const ack = await sendRuntime("OZ_ATTACHMENT_READY", { ...ownerPayload(recovery), actor_id: recovery.attach_commit_actor_id || runtime.id, attached_filenames: descriptors.map((item) => item.filename), baseline_assistant_turn_ids: assistantTurnIds() });
  ...
  async function attachClaimed(recovery) {
    ...
    const commit = await sendRuntime("OZ_ATTACHMENT_COMMIT", { ...ownerPayload(recovery), actor_id: runtime.id });
    ...
    const surface = active.attachmentSurface?.();
    if (!surface?.input) throw Object.assign(new Error("Target AI file-input attachment surface is unavailable."), { code: "TARGET_AI_ATTACHMENT_SURFACE_UNAVAILABLE" });
    OzonWebFileAttachment.setInputFiles(surface.input, files);
    const ready = await waitAttachmentReady(active, descriptors, ATTACH_READY_TIMEOUT_MS);
    if (!ready) throw Object.assign(new Error("Target AI did not confirm attachment-ready state within the bounded wait."), { code: "ATTACHMENT_READY_TIMEOUT" });
    stageMarker(committed);
    const acknowledged = await sendRuntime("OZ_ATTACHMENT_READY", { ...ownerPayload(committed), actor_id: runtime.id, attached_filenames: descriptors.map((item) => item.filename), baseline_assistant_turn_ids: assistantTurnIds() });
  ...
  async function sendReadyAttachment(recovery) {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      const target = await waitSendTarget(active, recovery.outgoing_text || "");
      if (!target) {
        lastError = Object.assign(new Error("Target AI Send control did not become stable for the attached delivery."), { code: "ATTACHMENT_SEND_TARGET_UNAVAILABLE" });
        continue;
      }
      const baseline = userTurnIds();
      const commit = await sendRuntime("OZ_ATTACHMENT_SEND_COMMIT", { ...ownerPayload(recovery), actor_id: runtime.id, baseline_user_turn_ids: baseline });
      ...
      try {
        ...
        if (error?.click_event_observed === false) {
          const rollback = await sendRuntime("OZ_ATTACHMENT_SEND_ROLLBACK", { ...ownerPayload(committedRecovery), actor_id: runtime.id, click_event_observed: false });
          if (rollback?.ok && rollback.rolled_back === true) {
            recovery = rollback.recovery || recovery;
  ...
  async function reconcileSendCommitted(recovery) {
    ...
    const confirmation = await sendRuntime("OZ_ATTACHMENT_CONFIRM", { ...ownerPayload(recovery), confirmed_user_turn_id: userTurnId, assistant_baseline_ids: assistantTurnIds() });
  ...
  async function processRecovery(initial) {
    ...
    try {
      for (;;) {
        ...
        switch (String(recovery.delivery_phase || "")) {
          ...
          case "attachment_committed":
            status("Ozon: восстанавливаю уже committed прикрепление без повторной загрузки…", "info", 0);
      ...
      if (String(recovery?.delivery_phase || "") === "attachment_send_committed" || error?.code === "ATTACHMENT_SEND_OUTCOME_UNKNOWN_NO_RETRY") {
        await failDelivery(recovery, error);
        return { ok: false, code: error.code || "ATTACHMENT_SEND_OUTCOME_UNKNOWN_NO_RETRY" };
      }
      await failDelivery(recovery, error);
  ...
  async function queryRecovery() {
    ...
    if (!key) return null;
    const response = await sendRuntime("OZ_ATTACHMENT_RECOVERY_GET", { conversation_key: key });
    return response?.ok ? response.recovery || null : null;
  }

  async function recoverCurrent() {
    const recovery = await queryRecovery();
    if (recovery) return processRecovery(recovery);
    return { ok: true, recovery: null };
  }

  messageListener = (message, _sender, sendResponse) => {
    if (message?.type !== "OZ_ATTACHMENT_DELIVERY_AVAILABLE") return false;
    const recovery = message.recovery || null;
    if (!recovery || recovery.conversation_key !== conversationKey()) {
      sendResponse({ ok: false, code: "ATTACHMENT_CONVERSATION_MISMATCH" });
      return false;
    }
    processRecovery(recovery).then(sendResponse).catch((error) => sendResponse({ ok: false, code: error?.code || "ATTACHMENT_CONTENT_ERROR", error: String(error?.message || error) }));
    return true;
  };
  ...
  identityPollTimer = setInterval(() => {
    ...
    queueMicrotask(() => { void recoverCurrent(); });
  }, 1000);

  setTimeout(() => { void recoverCurrent(); }, 500);
