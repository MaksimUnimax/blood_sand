...
**user → preferred AI → Ozon Bridge → Ozon cabinet data + public web/context + AI analysis → business answer / requested deliverable**
...
- `Сопоставь рекламные расходы, продажи, возвраты и себестоимость и покажи реальную прибыль по SKU.`
- `Сделай результат Excel-файлом с итоговым листом и графиком.`
...
The commercial core must test three distinct product-value classes:

1. **Decision / investigation** — explain what happened, why, what is risky, what to do next.
2. **Instant BI / ad-hoc analytics** — return a requested slice, ranking, comparison or summary without making the user download and join reports manually.
3. **Cross-report correlation** — combine data that Ozon exposes in different API surfaces, reports or accounting dates into one business view.
...
The default AI-product target is a **signed-in user on the AI provider's zero-cost/no-subscription web tier**. Paid AI subscriptions must not be assumed for baseline commercial coverage. Ozon Premium entitlements are separate from AI-provider subscription tiers.
...
Special attention must be paid to work that currently requires the seller/manager to download multiple XLS/XLSX/CSV reports, align SKU/article identifiers, reconcile different dates, or manually join Seller API and Performance/advertising data. Removing this manual joining work is itself a sellable product outcome.
...
The core must also contain representative **simple analytics** requests because speed/convenience is commercial value even when no diagnosis is required. Examples: period sales, top SKU, advertising spend, stock by warehouse, finance deductions, returns, supply status.
...
Example: cross-report correlation.

A question such as `Почему выплаты не сходятся с продажами за этот же период?` may require the worker to recognize that order, delivery, accrual and payment dates belong to different accounting views, retrieve the relevant Seller/finance evidence and reconcile them without forcing the user to download and manually join several reports.
...
## 11. Deliverable/output capability is part of product coverage

The worker must not be evaluated only on whether it can state the correct answer in chat. Users may want the same analysis as a reusable artifact.

Required output benchmark classes:

- concise chat answer;
- Markdown/plain table;
- chart/graph/visualization;
- downloadable CSV;
- downloadable XLSX/spreadsheet where the provider supports it;
- PDF report;
- DOCX/MD/text report where the provider supports it;
- PPTX/slide deck for client-facing reporting where the provider supports it;
- machine-readable JSON;
- machine-readable XML.

For each AI provider, distinguish:

- `NATIVE_FREE_SUPPORTED` — documented on signed-in zero-cost tier;
- `TEXT_ONLY_SUPPORTED` — can return structured text but no proven native downloadable artifact;
- `PAID_OR_CREDIT_GATED` — requires provider subscription/credits and therefore is not baseline product coverage;
- `UNVERIFIED` — must be browser-tested before being claimed.

A provider may PASS the business question but PARTIAL/FAIL the requested deliverable format. Both results must be preserved separately.

## 12. Free AI baseline

Commercial baseline assumes:

- user is authenticated in the AI web product;
- user has no paid AI subscription;
- normal free-tier rate/feature limits are accepted;
- paid AI-only artifact features are not counted as baseline capability;
- if a free feature can temporarily be unavailable under provider load, record that operational constraint;
- Ozon Premium/Premium Plus/Premium Pro is evaluated independently as seller-data entitlement.

This matters because the initial proposition is intended to work for a seller using the AI they already use, without requiring another AI subscription merely to obtain the core product value.

## 13. Product-gate decision after benchmark
...
- What exact analytics/correlation jobs eliminate manual report work?
- Which output formats can each free AI tier reliably produce?
...
## 14. Current authoritative product framing

> Each customer gets their own Ozon AI worker inside the AI they already prefer to use. The worker can be asked ordinary business questions about the seller cabinet. Ozon Bridge gives the AI controlled access to cabinet data; the AI decides what information to request, combines it with analysis and external context where useful, and returns a business answer or requested report instead of forcing the user to navigate Ozon reports, download files and join data manually.
