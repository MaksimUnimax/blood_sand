...
| 10 | STD-10 | Авария/пожар на складе Ozon: был ли там мой товар и что контролировать? | REOPENED_IN_PROGRESS — historical stock/damage reconstruction | PASS_FIRST_11_PROVIDER_READS_BUT_DAMAGE_QUESTION_NOT_YET_CLOSED | YES_REOPENED_AFTER_OPERATOR_CHALLENGE | 11 | Exact incident warehouse, pre-incident seller FBO flow and current sampled-SKU Samara zero are proven. Run5 found zero `compensation` transactions. Run6/6B found zero formal Samara removal/utilization rows. Runs7–9 are three terminal FBO windows covering local 2026-08-22..09-02 and contain zero postings from `САМАРА_РФЦ` / `23128509046000`, so ordinary outbound does not explain current Samara zero. Run10 `report_list` returned HTTP200 with `reports=[]`, `total=0`, proving there is no pre-existing report path to recover the historical baseline. Current Ozon API has historical placement report creation, but Bridge v0.1.19 does not expose creation + file ingestion; capability requirement recorded. Exact pre-incident Samara stock remains unknown. Next: `returns_list` filtered to FBO + Samara + post-incident logistic-return date, then remaining inbound/adjustment evidence. |
...
`PRIMARY_GATE_43_STD_10_REOPENED_RUN10_REPORT_LIST_TOTAL_ZERO_HISTORICAL_REPORT_CAPABILITY_GAP_SAMARA_FBO_RETURNS_NEXT_STD_12_PAUSED`
