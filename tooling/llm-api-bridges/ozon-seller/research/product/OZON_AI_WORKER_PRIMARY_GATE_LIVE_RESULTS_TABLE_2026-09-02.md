...
| 10 | STD-10 | Авария/пожар на складе Ozon: был ли там мой товар и что контролировать? | REOPENED_IN_PROGRESS — historical stock/damage reconstruction | PASS_FIRST_8_PROVIDER_READS_BUT_DAMAGE_QUESTION_NOT_YET_CLOSED | YES_REOPENED_AFTER_OPERATOR_CHALLENGE | 8 + reopened investigation | Prior runs proved exact incident warehouse, pre-incident seller FBO flow and current Samara zero for sampled SKUs. Run5 found no `compensation` transactions. Run6/6B proved no `removal_from_stock_list` rows from `САМАРА_РФЦ` in 2026-08-22..09-02. Run7 terminal FBO posting window covering local Samara 2026-08-22..25 (`has_next=false`) contains zero postings from `САМАРА_РФЦ` / `23128509046000`, so ordinary outbound FBO flow from Samara does not explain current zero during the first four incident/post-incident calendar days. Exact pre-incident Samara stock remains the load-bearing baseline gap. Next: local 2026-08-26..29 FBO posting window. |
...
`PRIMARY_GATE_43_STD_10_REOPENED_RUN7_SAMARA_POSTINGS_WINDOW1_ZERO_WINDOW2_NEXT_STD_12_PAUSED`
