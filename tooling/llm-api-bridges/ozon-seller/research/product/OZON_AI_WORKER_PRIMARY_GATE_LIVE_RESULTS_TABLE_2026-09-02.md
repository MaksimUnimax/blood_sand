...
| 10 | STD-10 | Авария/пожар на складе Ozon: был ли там мой товар и что контролировать? | REOPENED_IN_PROGRESS — historical stock/damage reconstruction | PASS_FIRST_5_READS_BUT_DAMAGE_QUESTION_NOT_YET_CLOSED | YES_REOPENED_AFTER_OPERATOR_CHALLENGE | 5 + reopened investigation | Prior runs proved exact incident warehouse, pre-incident seller FBO flow, current Samara zero for sampled SKUs, and healthy current stock elsewhere. Reopened Run5 `finance_transaction_list_v3` for 2026-08-22..09-02 with `transaction_type=compensation` returned HTTP200, `operations=[]`, `page_count=0`, `row_count=0`: no compensation-classified finance transactions in the window, but this does not prove zero loss/write-off. Current registered stock APIs remain current-only with no historical `as_of`; exact pre-incident Samara stock is still load-bearing. Next: `removal_from_stock_list` 2026-08-22..09-02, limit500, then post-incident Samara postings and historical baseline attempt. |
...
`PRIMARY_GATE_43_STD_10_REOPENED_RUN5_NO_COMPENSATION_ROWS_REMOVAL_UTILIZATION_READ_NEXT_STD_12_PAUSED`
