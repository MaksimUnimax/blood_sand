...
| 10 | STD-10 | Авария/пожар на складе Ozon: был ли там мой товар и что контролировать? | REOPENED_IN_PROGRESS — historical stock/damage reconstruction | PASS_FIRST_9_PROVIDER_READS_BUT_DAMAGE_QUESTION_NOT_YET_CLOSED | YES_REOPENED_AFTER_OPERATOR_CHALLENGE | 9 + reopened investigation | Prior runs proved exact incident warehouse, pre-incident seller FBO flow and current Samara zero for sampled SKUs. Run5 found no `compensation` transactions. Run6/6B proved no formal removal/utilization rows from `САМАРА_РФЦ` in 2026-08-22..09-02. Run7 terminal local window 2026-08-22..25 found zero Samara FBO postings. Run8 terminal local window 2026-08-26..29 also found zero Samara FBO postings. Thus local 2026-08-22..29 ordinary FBO outbound from Samara is zero. Exact pre-incident Samara stock remains the load-bearing baseline gap. Next: final local 2026-08-30..09-02 FBO posting window, then historical baseline/remaining movements. |
...
`PRIMARY_GATE_43_STD_10_REOPENED_RUN8_SAMARA_POSTINGS_LOCAL_22_TO_29_ZERO_FINAL_WINDOW_30_TO_SEP2_NEXT_STD_12_PAUSED`
