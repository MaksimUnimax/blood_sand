...
| 10 | STD-10 | Авария/пожар на складе Ozon: был ли там мой товар и что контролировать? | REOPENED_BLOCKED — read-classification false negatives discovered | PASS_FIRST_11_PROVIDER_READS_THEN_READ_SURFACE_CLASSIFICATION_DEFECT_DISCOVERED | YES_REOPENED_AFTER_OPERATOR_CHALLENGE | 11 | Exact incident warehouse, pre-incident seller FBO flow and current sampled-SKU Samara zero are proven. Run5 found zero `compensation` transactions. Run6/6B found zero formal Samara removal/utilization rows. Runs7–9 contain zero postings from `САМАРА_РФЦ` / `23128509046000`, so ordinary outbound does not explain current Samara zero. Run10 `report_list` returned `reports=[]`, `total=0`. Re-audit of the final 463-row terminal matrix found that the generic `REJECT_SERVER_SIDE_GENERATION_OR_CREATION` rule incorrectly removed passive report/document/label/validation workflows from READ coverage; at least 26 false-negative READs are confirmed, including `POST /v1/report/placement/by-products/create`. `POST /v3/chat/history` was also wrongly removed instead of being handled by the Personal Data gate. Under `NO_SKIP_ON_FAILURE`, stop STD-10 progression and reclassify/fix the READ surface before treating the historical baseline as unavailable. Evidence: `OZON_AI_WORKER_READ_CLASSIFICATION_REAUDIT_2026-09-02.md`. |
...
`PRIMARY_GATE_43_STD_10_BLOCKED_READ_CLASSIFICATION_REAUDIT_AT_LEAST_26_FALSE_NEGATIVE_READS_FIX_BEFORE_CONTINUING_STD_12_PAUSED`
