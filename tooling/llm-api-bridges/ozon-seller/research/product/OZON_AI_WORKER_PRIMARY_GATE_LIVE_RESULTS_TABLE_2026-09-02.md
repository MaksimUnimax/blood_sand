...
| 10 | STD-10 | Авария/пожар на складе Ozon: был ли там мой товар и что контролировать? | PASS_WITH_EXPLICIT_INCIDENT_CAUSALITY_AND_HISTORICAL_SNAPSHOT_LIMITS | PASS_ALL_STD10_PROVIDER_READS | NO | 4 | Run1 matched the 2026-08-22 Chapayevsk incident to `САМАРА_РФЦ` (`23128509046000`). Run2 proved pre-incident seller exposure: `2271188511` Лев (Символы) delivered; `1720141903` Водолей Ozon-cancelled reason 695 `Не удалось доставить заказ`. Run3 explicitly returned Samara `present=0,reserved=0` for both exposed SKUs. Run4 fresh aggregate state: both cards `Продается`, approved, validation success, `errors=[]`, `AVAILABLE`; Лев = FBO4 + FBS43, total free47; Водолей = FBO1 reserved1 + FBS43, total free43. No current total-stock loss is supported; current concern is Водолей FBO allocation. Exact in-building stock at incident time, physical damage/loss and causal compensation/write-off remain unproven coverage limits. |
| 11 | STD-11 | Исчез товар с FBO без продаж: куда мог деться? | PASS — free-FBO disappearance explained by exact active-order reservation | PASS_FIRST_PROVIDER_READ | NO | 1 | Real same-day case SKU `1720141903` Водолей: free FBO changed 1→0 while `present` stayed 1 and `reserved` changed 0→1. Run1 found active posting `0223728377-0109-5`, qty1, `awaiting_packaging/posting_created`, exactly at warehouse `1020001007805000` = `ВОРОНЕЖ_2_РФЦ`, the sole non-zero FBO warehouse with `present=1,reserved=1`. Therefore free stock was reserved for an active order, not physically lost and not yet a completed sale. Semantic rule: `FREE_FBO_DECREASE_IS_NOT_EQUIVALENT_TO_PHYSICAL_FBO_LOSS_OR_COMPLETED_SALE`. STD-12 READY. |
| 12 | STD-12 | Какие поставки сейчас активны и что с каждой происходит? | READY | PENDING | PENDING | 0 | Next active Standard business row under `NO_SKIP_ON_FAILURE`. |
