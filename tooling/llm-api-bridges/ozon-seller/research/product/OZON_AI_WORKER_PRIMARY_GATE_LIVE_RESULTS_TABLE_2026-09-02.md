...
| 10 | STD-10 | Авария/пожар на складе Ozon: был ли там мой товар и что контролировать? | IN_PROGRESS — historical exposure proven; current Samara stock zero for both exposed SKUs | PASS_RUN1_TO_RUN3_PROVIDER_READS | NO | 3 | Run1 matched the 2026-08-22 Chapayevsk incident to `САМАРА_РФЦ` (`23128509046000`). Run2 found two pre-incident seller SKUs routed from Samara: `2271188511` Лев (Символы), delivered; `1720141903` Водолей, Ozon-cancelled reason 695 `Не удалось доставить заказ`. Run3 focused current FBO placement explicitly returns Samara `present=0,reserved=0` for both. Лев has 4 FBO units elsewhere (0 reserved); Водолей has only 1 FBO unit and it is reserved. Earlier same-day STD-07 had Водолей FBS43 but its FBO reservation state has already changed, so stale aggregate totals cannot be reused as exact current state. Next: fresh `seller_product_info_list` for both exposed SKUs to establish current total FBO+FBS and sellability. |
...
`PRIMARY_GATE_43_BASELINE_EXPANDABLE_STD_10_RUN3_SAMARA_ZERO_EXPOSED_SKUS_CURRENT_TOTAL_PRODUCT_STATE_NEXT`
