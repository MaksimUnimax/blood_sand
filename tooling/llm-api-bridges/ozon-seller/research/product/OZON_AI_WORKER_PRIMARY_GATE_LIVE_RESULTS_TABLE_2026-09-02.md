...
| 10 | STD-10 | Авария/пожар на складе Ozon: был ли там мой товар и что контролировать? | REOPENED_IN_PROGRESS — historical stock/damage reconstruction | PASS_FIRST_4_READS_BUT_DAMAGE_QUESTION_NOT_YET_CLOSED | YES_REOPENED_AFTER_OPERATOR_CHALLENGE | 4 + reopened investigation | Prior runs proved exact incident warehouse, pre-incident seller FBO flow, current Samara zero for sampled SKUs, and healthy current stock elsewhere. Closure was premature because this did not determine whether physical/accounted inventory was lost in the incident. Reopened plan: obtain/derive pre-incident Samara stock baseline, subtract normal post-incident outflows (sales/postings, removals/utilization, returns/supplies/transfers where observable), compare with current Samara zero, and correlate residual with compensation/write-off evidence. Current registered stock APIs are current-only and expose no historical `as_of`; historical placement/report capability is a recorded Bridge coverage gap. Next: `finance_transaction_list_v3` for `transaction_type=compensation`, 2026-08-22..2026-09-02. |
...
`PRIMARY_GATE_43_STD_10_REOPENED_HISTORICAL_STOCK_DAMAGE_RECONSTRUCTION_STD_12_PAUSED_COMPENSATION_READ_NEXT`
